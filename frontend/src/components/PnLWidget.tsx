  ...
  const loadPnL = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Safe fallback: if response status = 500 → return empty trades array
        if (tradesErr.response?.status === 500) {
          console.warn('[PnLWidget] Trades API returned 500 error, using empty array fallback');
          trades = [];
        } else {
          throw tradesErr; // Re-throw if not 500
        }
      }
      ...
      try {
        ...
        if (execErr.response?.status === 500) {
          console.warn('[PnLWidget] Execution logs API returned 500 error, skipping accuracy calculation');
          setAccuracy(0);
        } else {
