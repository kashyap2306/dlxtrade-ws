...
export default function MarketScanner() {
  ...
  const fetchTopCoins = useCallback(async () => {
    ...
    try {
      ...
      // Log full API response for confirmation
      console.log('MarketScanner API Response:', data);

      // NORMALIZE: Prioritize 'topMovers' or 'movers'
      const rawList: any[] =
        Array.isArray(data?.topMovers) ? data.topMovers :
          Array.isArray(data?.movers) ? data.movers :
      ...
      if (!Array.isArray(rawList)) {
        console.warn('MarketScanner: Normalized data is not an array', rawList);
        setCoins([]);
        return;
      }

      // Transform backend data to our format
      const topCoins = rawList
        .map((coin: any) => ({
          symbol: coin.symbol || '',
          price: parseFloat(coin.price || 0),
          // Use normalized field names from backend
          priceChange24h: parseFloat(coin.priceChange24h || coin.priceChangePercent24h || 0),
          volume24h: parseFloat(coin.volume24h || 0),
          pumpPercent: parseFloat(coin.priceChange24h || coin.priceChangePercent24h || 0),
        }))
      ...
      console.log('MarketScanner Render Coins:', topCoins);
      setCoins(topCoins);
    } catch (err: any) {
