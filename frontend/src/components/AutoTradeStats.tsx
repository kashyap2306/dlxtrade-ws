    ...
    <div className="bg-[#0a0f1a] backdrop-blur-sm border border-blue-500/20 rounded-xl p-6 mb-8 shadow-lg">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div>
          ...
          <div className={`text-xl font-bold ${(performanceStats?.dailyPnL ?? 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
            ${(performanceStats?.dailyPnL ?? 0).toFixed(2)}
          </div>
        </div>
        <div>
          <div className="text-sm text-blue-100/60">All Time</div>
          <div className={`text-xl font-bold ${(performanceStats?.allTimePnL ?? 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
        ...
        <div>
          <div className="text-sm text-blue-100/60">Active Trades</div>
