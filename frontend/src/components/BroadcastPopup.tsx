...
export default function BroadcastPopup() {
  ...
  useEffect(() => {
    ...
    const loadPopup = async () => {
      ...
      if (isLoading) {
        console.log('[BroadcastPopup] Skipping - previous call still in progress');
        return;
      }

      // Stop polling if too many consecutive errors
      if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
        console.warn('[BroadcastPopup] Stopping polling due to too many consecutive errors');
        if (pollingInterval) {
      ...
      try {
        ...
        consecutiveErrors++;
        console.error(`Error loading broadcast popup (${consecutiveErrors}/${MAX_CONSECUTIVE_ERRORS}):`, error);
