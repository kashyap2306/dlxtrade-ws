...
interface UnlockFormModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (formData: { fullName: string; phoneNumber: string; email: string }) => void;
}
...
export default function UnlockFormModal({ agent, isOpen, onClose, onSuccess }: UnlockFormModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // Call onSuccess callback which will handle Firestore write
      if (onSuccess) {
        onSuccess({
        ...
        // Fallback: Submit purchase request to backend
