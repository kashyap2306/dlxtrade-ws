    ...
    >
      ...
      <div className="relative h-48 overflow-hidden bg-gradient-to-br from-purple-500/20 to-pink-500/20">
        ...
        <div className={`absolute top-3 left-3 px-3 py-1.5 rounded-lg text-xs font-semibold backdrop-blur-md
          ${isUnlocked ? 'bg-green-500/20 border border-green-500/50 text-green-300' : 
            agent.requestStatus === 'pending' ? 'bg-yellow-500/20 border border-yellow-500/50 text-yellow-300' :
            'bg-slate-700/80 border border-slate-500/50 text-gray-300'}`}>
          {isUnlocked ? '✓ Active' : 
           agent.requestStatus === 'pending' ? '⏳ Pending' :
           '🔒 Locked'}
        </div>
            ...
            {!isUnlocked ? (
              <>
                {agent.requestStatus === 'pending' ? (
                  <button
                    disabled
                    className="flex-1 px-4 py-2 text-sm font-semibold rounded-lg bg-yellow-500/20 border border-yellow-500/50 text-yellow-300 cursor-not-allowed"
                  >
                    ⏳ Awaiting Approval
                  </button>
                ) : (
