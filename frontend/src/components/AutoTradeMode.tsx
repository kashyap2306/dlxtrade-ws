...
export default function AutoTradeMode({ onStatusChange }: AutoTradeModeProps) {
  ...
  // If the enabled state or API connection status is still loading (null), show a loading placeholder
  if (enabled === null || isApiConnected === null) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white"></div>
        <span className="ml-2 text-white">Loading Auto‑Trade status…</span>
      </div>
    );
  }
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-xl border border-purple-500/20 bg-black/40">
              ...
              <div className={`text-sm font-semibold ${enabled ? 'text-green-400' : (enabled === false ? 'text-gray-400' : 'text-yellow-400')
                }`}>
          ...
          <button
            ...
            disabled={loading || checking || enabled === null || (!enabled && !canEnable)}
            className={`w-full px-6 py-4 rounded-xl font-semibold text-lg transition-all duration-300 shadow-lg ${enabled
