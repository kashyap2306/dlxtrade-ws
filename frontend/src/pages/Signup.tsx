...
export default function Signup() {
  ...
  const handleAfterSignUp = async (idToken: string, userCredential: any) => {
    try {
      ...
      // CRITICAL: Call backend POST /api/auth/afterSignIn with idToken
      // This triggers ensureUser() which creates ALL Firestore documents atomically:
      // - users/{uid} document
      // - users/{uid}/integrations/* documents (via seedProviderIntegrations)
      // - engineStatus/{uid}
      // - uiPreferences/{uid}
      // - All other required documents
      // The /users/complete-signup endpoint is redundant (ensureUser already seeds integrations)
      // but we keep it for backward compatibility - if it fails, signup still succeeds
      ...
      try {
        ...
        if (!afterSignInResponse.data?.success) {
          console.warn('User onboarding returned success=false, but signup continues (auth succeeded)');
        }
      } catch (afterSignInErr: any) {
        // CRITICAL: Do NOT block signup - auth succeeded, user can log in
        // Log error but continue to onboarding
        console.warn('afterSignIn endpoint failed (non-blocking):', afterSignInErr.message);
        // User can still log in - auth succeeded
      }

      // Optional: Call complete-signup for backward compatibility (non-blocking)
      // This endpoint is redundant since ensureUser() already seeds integrations
      // If it fails, we don't block signup since the user document is already created
      try {
        console.log("completeSignUp sent (optional)");
        await api.post('/users/complete-signup', { idToken });
      } catch (completeSignupErr: any) {
        // Log but don't block - ensureUser() already created everything needed
        console.warn('complete-signup endpoint failed (non-critical):', completeSignupErr.message);
      }

      // CRITICAL: Signup always succeeds if auth succeeded
      // New users always go to onboarding
      navigate('/onboarding');
    } catch (err: any) {
      // CRITICAL: Only show error if it's an auth error (before handleAfterSignUp)
      // If we reach here, it means auth itself failed, not post-auth setup
