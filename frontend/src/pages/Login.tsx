...
export default function Login() {
  ...
  const handleAfterSignIn = async (userCredential: any) => {
    try {
      ...
      // Use React Router navigation to prevent blinking/page reload
      navigate(targetPath, { replace: true });
    } catch (err: any) {
      suppressConsoleError(err, 'completeSignIn');
      const { message, type } = getFirebaseErrorMessage(err);
      showError(message, type);
      // Add login failure notification
      await addNotification({
        title: 'Login Failed',
        message: message || 'Failed to sign in. Please check your credentials.',
        type: 'error',
      });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      suppressConsoleError(err, 'login');
      const { message, type } = getFirebaseErrorMessage(err);
      showError(message, type);
    } finally {
      setLoading(false);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      suppressConsoleError(err, 'googleLogin');
      const { message, type } = getFirebaseErrorMessage(err);
      showError(message, type);
    } finally {
      setLoading(false);
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      suppressConsoleError(err, 'githubLogin');
      const { message, type } = getFirebaseErrorMessage(err);
      showError(message, type);
    } finally {
      setLoading(false);
  ...
  const handleForgotPassword = async () => {
    try {
      if (!email) {
        showError('Enter your email to reset password', 'validation');
        return;
      }
      await sendPasswordResetEmail(auth, email);
      showToast('Password reset email sent. Check your inbox.', 'success');
    } catch (err: any) {
      suppressConsoleError(err, 'forgotPassword');
      const { message, type } = getFirebaseErrorMessage(err);
      showError(message, type);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900/20 to-slate-900 relative overflow-hidden px-4">
      ...
      <div className="relative card max-w-md w-full p-6 sm:p-8 mx-4 z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className="w-full text-sm text-purple-300 hover:text-purple-200 transition-colors text-right"
            disabled={loading}
          >
            Forgot Password?
          </button>
