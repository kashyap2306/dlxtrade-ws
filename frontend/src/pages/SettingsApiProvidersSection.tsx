...
export const SettingsApiProvidersSection: React.FC<SettingsApiProvidersSectionProps> = ({
  ...
  const isProviderSaved = (providerName: string) => {
    const providerId = (PROVIDER_ID_MAP[providerName] || '').toLowerCase();
    return apiKeys[providerId]?.saved ?? false;
  };
  ...
  const saveApiKey = async (providerName: string) => {
    if (providerName === 'CryptoCompare') {
      console.log("🔥 CRYPTOCOMPARE SAVE CLICKED");
    }
    ...
    if (providerName === 'CryptoCompare') {
      console.log("🔥 CRYPTOCOMPARE CALLING API", { providerId, apiKeyLength: settings[providerKey]?.length });
    }
    ...
    // Don't clear the settings key - keep it for the UI during editing
    // The provider will reload from backend after save
    setEditingProvider(null);
    setOriginalApiKey('');
  };
            ...
            <div className="mb-6 p-4 bg-slate-800/30 rounded-xl border border-purple-500/30">
              ...
              {config.primary.name !== 'CoinGecko' ? (
                <div className="space-y-3">
                    {(() => {
                    const isSaved = isProviderSaved(config.primary.name);
                    const isEditing = editingProvider === config.primary.name || !isSaved;
                      ...
                      return (
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-slate-900/40 p-3 rounded-lg border border-white/10">
                          <div>
                            ...
                            <div className="text-xs text-gray-400">API key saved</div>
                          </div>
                    ...
                    return (
                      <>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 sm:justify-end">
                          ...
                          >
                            {savingProvider === config.primary.name ? 'Saving...' : isSaved ? 'Update' : 'Save'}
                          </button>
                          {isSaved && (
                            <button
                  ...
                  <div key={backup.name} className="p-3 bg-slate-800/30 rounded-xl border border-white/10">
                    ...
                    {settings[backup.enabledKey] && backup.type === 'api' && (
                      <div className="space-y-3 mt-3">
                        {(() => {
                          const isSaved = isProviderSaved(backup.name);
                          const isEditing = editingProvider === backup.name || !isSaved;
                            ...
                            <>
                              ...
                              <div className="flex flex-col sm:flex-row gap-2 sm:justify-end">
                                <button
                                  ...
                                  className="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-500 text-white font-medium rounded-lg text-sm hover:from-green-600 hover:to-emerald-600 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300 disabled:opacity-50 hover:scale-[1.05]"
                                >
                                  {savingProvider === backup.name ? 'Saving...' : isSaved ? 'Update' : 'Save'}
                                </button>
                                {isSaved && (
                                  <button
                                    onClick={cancelEditApi}
