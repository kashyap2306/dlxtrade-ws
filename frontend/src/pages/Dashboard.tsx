...
export default function Dashboard() {
  ...
  const handleTestExchange = async () => {
    ...
    try {
      ...
      const response = await exchangeApi.testExchangeConnection(testConfig);
      ...
      <ErrorBoundary>
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={api.name} className="flex items-center justify-between py-2">
                        ...
                        <div className={`w-2 h-2 rounded-full ${api.status === 'connected' ? 'bg-emerald-400' : 'bg-red-400'
                          }`}></div>
                      </div>
                    ))}
                  </div>
              ...
              {alerts.length > 0 && (
                <div className="mb-8 space-y-4">
                  {alerts.map((alert, index) => (
                    <div
                      key={index}
                      className={`p-5 rounded-xl border backdrop-blur-sm ${alert.type === 'error'
                        ? 'bg-red-500/10 border-red-500/30'
                        : 'bg-yellow-500/10 border-yellow-500/30'
                        } hover:shadow-lg transition-all duration-300`}
                    >
                      <div className="flex items-start gap-4">
                        <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${alert.type === 'error' ? 'bg-red-500/20' : 'bg-yellow-500/20'
                          }`}>
                          {alert.type === 'error' ? (
                        ...
                        <div className="flex-1">
                          <p className={`font-medium ${alert.type === 'error' ? 'text-red-300' : 'text-yellow-300'
                            }`}>
                            {alert.message}
                          </p>
                        </div>
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 hover:shadow-lg hover:shadow-slate-900/20 transition-all duration-300">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-4 bg-slate-700/30 rounded-xl">
                      <div className={`text-2xl font-bold mb-1 ${(performanceStats?.dailyPnL || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'
                        }`}>
                    ...
                    <div className="text-center p-4 bg-slate-700/30 rounded-xl">
                      <div className={`text-2xl font-bold mb-1 ${(performanceStats?.allTimePnL || 0) >= 0 ? 'text-emerald-400' : 'text-red-400'
                        }`}>
                        ...
                        return (
                          <div key={trade.tradeId || index} className="flex items-center justify-between p-3 bg-slate-700/30 rounded-lg">
                            ...
                            <div className="text-right">
                              <div className={`font-medium text-sm ${trade.side === 'buy' ? 'text-emerald-400' : 'text-red-400'
                                }`}>
                    ...
                    {aiSignals.map((signal: any, index: number) => (
                      <div key={signal.id || index} className="p-4 bg-slate-700/30 rounded-xl">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${signal.direction === 'BUY'
          ...
          <TradeConfirmationModal
            ...
            side={tradeConfirmation.tradeData?.side}
            entryPrice={tradeConfirmation.tradeData?.entryPrice}
            stopLoss={tradeConfirmation.tradeData?.stopLoss}
            requestId={tradeConfirmation.tradeData?.requestId || 'manual-confirm'}
