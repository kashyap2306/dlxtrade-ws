        ...
        <div className="flex items-center justify-between mb-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 flex-1">
            ...
            <div>
              ...
              {(() => {
                const acc = Math.max(0, Math.min(1, liveData.accuracy ?? 0));
                const displayPercent = Math.min(100, Math.round(acc * 100));

                // SAFETY ASSERT
                if (displayPercent > 100) {
                  console.error("ACCURACY CONTRACT VIOLATION in LiveResearchCard", liveData.accuracy);
                }

                return (
                  <div className={`font-semibold text-lg ${acc >= 0.85 ? 'text-green-400' :
                    acc >= 0.7 ? 'text-yellow-400' :
                      'text-red-400'
                    }`}>
                    {displayPercent}%
                  </div>
