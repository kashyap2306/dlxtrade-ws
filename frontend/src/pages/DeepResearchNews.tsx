...
const DeepResearchNews: React.FC<DeepResearchNewsProps> = ({
  ...
  // CRITICAL: Check multiple locations for news - backend may provide it in different places
  // Check reactively - data may arrive late
  const newsData = result.news || 
                   (result as any)?.result?.news ||
                   { articles: [] };
  
  return (
    <>
      {/* NEWS IMPACT SUMMARY */}
      {newsData?.articles && Array.isArray(newsData.articles) && newsData.articles.length > 0 && (
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="text-right">
                ...
                <div className="text-lg font-bold text-white">{newsData.articles.length}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-600/30 rounded-xl p-4">
                ...
                <div className="text-2xl font-bold text-green-400">
                  {newsData.articles.filter((article: any) => article.sentiment === 'positive').length}
                </div>
              ...
              <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-600/30 rounded-xl p-4">
                ...
                <div className="text-2xl font-bold text-slate-400">
                  {newsData.articles.filter((article: any) => article.sentiment === 'neutral').length}
                </div>
              ...
              <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-600/30 rounded-xl p-4">
                ...
                <div className="text-2xl font-bold text-red-400">
                  {newsData.articles.filter((article: any) => article.sentiment === 'negative').length}
                </div>
              ...
              <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-600/30 rounded-xl p-4">
                ...
                <div className={`text-2xl font-bold ${(() => {
                  ...
                  const total = newsData.articles.length;
        ...
        <div className="relative bg-gradient-to-br from-slate-900/90 via-slate-800/90 to-slate-900/90 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-4 sm:p-6 shadow-2xl shadow-slate-900/50 overflow-hidden">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="text-right">
                <div className="text-sm text-slate-400">{newsData.articles.length} Articles</div>
