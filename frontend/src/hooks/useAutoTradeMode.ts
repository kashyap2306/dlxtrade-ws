...
export function useAutoTradeMode(): UseAutoTradeModeReturn {
  const { user } = useAuth();
  // Use null as initial state to indicate unknown until backend fetch completes
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      // Persisted value may be true/false; if undefined treat as false
      ...
      // On error, keep unknown state as false to avoid flashing ON
      setEnabled(false);
  ...
  return {
    // enabled may be null initially; UI should treat null as loading
    enabled,
    isApiConnected,
