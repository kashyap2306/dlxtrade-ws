...
if (import.meta.env.DEV) {
  setTimeout(async () => {
    ...
    try {
      ...
      const { auth, isFirebaseAvailable, isFirebaseReady } =
      ...
      console.log("✅ Axios interceptor active:", !!api.interceptors?.request);
      ...
      console.log(
        "[RUNTIME CHECK] typeof isFirebaseAvailable:",
        typeof isFirebaseAvailable,
      );
      console.log("[RUNTIME CHECK] typeof getAuthToken:", typeof getAuthToken);
      console.log(
        "[RUNTIME CHECK] api.defaults.baseURL:",
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      const { testAgentsMarketplace } =
        await import("./tests/test-agents-marketplace");
