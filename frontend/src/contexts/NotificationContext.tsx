...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    if (isLoadingRef.current) {
      console.log('[Notifications] Skipping - previous call still in progress');
      return;
    }

    // Stop polling if too many consecutive errors
    if (consecutiveErrorsRef.current >= MAX_CONSECUTIVE_ERRORS) {
      console.warn('[Notifications] Stopping polling due to too many consecutive errors');
    ...
    try {
      ...
      lastNotificationIdsRef.current = currentIds as Set<string>;
      ...
      console.debug(`Error loading notifications (${consecutiveErrorsRef.current}/${MAX_CONSECUTIVE_ERRORS}):`, err);
