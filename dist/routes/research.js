...
async function researchRoutes(fastify) {
    ...
    fastify.post('/run', {
        ...
        try {
            logger_1.logger.info({ uid: user.uid }, 'Starting deep research with CryptoQuant + LunarCrush + CoinAPI');
            ...
            logger_1.logger.info({ uid: user.uid, symbols }, 'Starting research API calls for symbols');
                ...
                try {
                    ...
                    if (hasLunarCrush) {
                        try {
                            ...
                            logger_1.logger.info({ uid: user.uid, symbol, requestId }, 'LunarCrush data fetched');
                        }
                        catch (err) {
                            logger_1.logger.error({ err: err.message, symbol, requestId }, 'LunarCrush API call failed');
                    ...
                    if (hasCoinAPIMarket) {
                        try {
                            ...
                            logger_1.logger.info({ uid: user.uid, symbol, requestId }, 'CoinAPI Market Data fetched');
                        }
                        catch (err) {
                            logger_1.logger.error({ err: err.message, symbol, requestId }, 'CoinAPI Market Data API call failed');
                    ...
                    if (hasCoinAPIExchangerate) {
                        try {
                            ...
                            logger_1.logger.info({ uid: user.uid, symbol, requestId }, 'CoinAPI Exchange Rate Data fetched');
                        }
                        catch (err) {
                            logger_1.logger.error({ err: err.message, symbol, requestId }, 'CoinAPI Exchange Rate API call failed');
                    ...
                    if (hasCoinAPIFlatfile) {
                        try {
                            ...
                            logger_1.logger.info({ uid: user.uid, symbol, requestId }, 'CoinAPI Flat Files Data fetched');
                        }
                        catch (err) {
                            logger_1.logger.error({ err: err.message, symbol, requestId }, 'CoinAPI Flat Files API call failed');
                    ...
                    if (coinApiMarketData.ohlcvData && coinApiMarketData.ohlcvData.length > 0) {
                        const closes = coinApiMarketData.ohlcvData.map((d) => d.close).filter((p) => p > 0);
                        if (closes.length >= 14) {
                    ...
                    const onChainScore = (cryptoQuantData.exchangeFlow && cryptoQuantData.exchangeFlow < 0 ? 1 : 0) * 0.1 + // Negative flow = bullish
                    ...
                    if (cryptoQuantData && !cryptoQuantData.error)
                        apiSuccessCount++;
                    if (lunarCrushData && !lunarCrushData.error)
                        apiSuccessCount++;
                    if (coinApiMarketData && !coinApiMarketData.error)
                        apiSuccessCount++;
                    if (coinApiExchangeRateData && !coinApiExchangeRateData.error)
                        apiSuccessCount++;
                    if (coinApiFlatFileData && !coinApiFlatFileData.error)
                        apiSuccessCount++;
                    ...
                    try {
                        ...
                        logger_1.logger.info({ uid: user.uid, symbol, requestId, type: 'manual', accuracy: confidencePercent }, 'Research log saved to Firestore');
                    }
                    catch (logErr) {
                        logger_1.logger.error({ err: logErr.message, symbol, requestId }, 'Failed to save research log');
                    }
                    const symbolLatency = Date.now() - symbolStartTime;
                    logger_1.logger.info({ uid: user.uid, symbol, latency: symbolLatency, signal, confidencePercent, requestId }, 'Deep research completed for symbol');
                    ...
                    logger_1.logger.error({ err: err.message, stack: err.stack, symbol, uid: user.uid }, 'Error in deep research for symbol');
                    results.push({
                        symbol,
            ...
            logger_1.logger.error({ error: error.message, stack: error.stack, uid: user.uid }, 'Error in deep research');
                ...
                try {
                    ...
                    if (hasCoinAPIMarket) {
                        try {
                            ...
                            logger_1.logger.debug({ err, symbol }, 'Could not fetch price from CoinAPI, using estimate');
                        }
                    }
                    let entry;
                    let size;
    ...
    async function fallbackAggregator(uid) {
        try {
            ...
            if (!coinapiMarket) {
                logger_1.logger.warn({ uid }, 'Fallback aggregator requires CoinAPI market integration');
            ...
            if (coinapiMarket) {
                try {
                    ...
                    logger_1.logger.error({ err, uid }, 'Error fetching coins from CoinAPI');
                }
            }
            ...
            for (const coin of topCoins) {
                try {
                    ...
                    const flowScore = onChainFlow > 0 ?
                        ...
                        0;
                    const accuracy = Math.min(Math.max(priceScore + volumeScore + socialScoreComponent + flowScore, 0.3 // Minimum accuracy
                    ), 0.95);
                    // Determine signal
                    const suggestion = (trendStrength > 0 && socialSentiment > 0 && onChainFlow > -5000000) ? 'BUY' : 'SELL';
                    const trendDirection = trendStrength > 2 ? 'UP' :
                    ...
                    logger_1.logger.error({ err, symbol: coin.symbol }, 'Error analyzing coin in fallback aggregator');
                }
            }
            ...
            logger_1.logger.error({ error: error.message, uid }, 'Error in fallback aggregator');
    ...
    fastify.post('/manual', {
        ...
        const user = request.user;
        const body = zod_1.z.object({
            selectedExchange: zod_1.z.enum(['binance', 'bitget', 'weex', 'bingx']).optional(), // Ignored - research APIs only
            symbols: zod_1.z.array(zod_1.z.string()).optional(),
            topN: zod_1.z.number().int().positive().max(10).optional().default(3),
        }).parse(request.body || {});
        try {
            ...
            if (!hasCryptoQuant && !hasLunarCrush && !hasCoinAPIMarket && !hasCoinAPIFlatfile && !hasCoinAPIExchangerate) {
                ...
                logger_1.logger.info({ uid: user.uid }, 'No research API credentials found, using fallback aggregator');
            ...
            let symbols = body.symbols || ['BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'SOLUSDT', 'ADAUSDT'];
            logger_1.logger.info({ uid: user.uid, symbolCount: symbols.length }, 'Starting manual deep research (research APIs only)');
            // Run research for each symbol using ONLY research APIs
            const candidates = [];
            for (const symbol of symbols) {
                try {
                    ...
                    if (hasCoinAPIFlatfile) {
                        try {
                            ...
                            logger_1.logger.debug({ err, symbol }, 'Could not calculate RSI/trend from CoinAPI, using defaults');
                        }
                    }
                    // Skip HOLD signals with low accuracy
                    if (research.signal === 'HOLD' && research.accuracy < 0.7)
                        continue;
                    // Calculate entry, exit, stop-loss, take-profit
                    ...
                    let suggestion = research.signal === 'BUY' ? 'BUY' : 'SELL';
                    if (research.signal === 'BUY') {
                        exit = entry * 1.04;
                        tp = entry * 1.04;
                        sl = entry * 0.98;
            ...
            if (!bestCandidate) {
                logger_1.logger.info({ uid: user.uid }, 'No candidates found, using fallback aggregator');
                const fallbackResult = await fallbackAggregator(user.uid);
                if (fallbackResult) {
                    const trendDescription = fallbackResult.trendStrength > 10
                        ? 'strong uptrend'
    ...
    fastify.get('/manual', {
        ...
        try {
            ...
            if (!hasCryptoQuant && !hasLunarCrush && !hasCoinAPIMarket && !hasCoinAPIFlatfile && !hasCoinAPIExchangerate) {
                // Use fallback aggregator if no research APIs
                logger_1.logger.info({ uid: user.uid }, 'Manual deep research called without research APIs, using fallback aggregator');
                const fallbackResult = await fallbackAggregator(user.uid);
                if (!fallbackResult) {
                    return reply.code(500).send({
                        error: 'Failed to generate fallback signal',
    ...
    fastify.post('/runOne', {
        ...
        try {
            logger_1.logger.info({ uid: body.uid, requestedBy: user.uid }, 'Running research for single user via runOne endpoint');
            ...
            logger_1.logger.error({ error: error.message, stack: error.stack, uid: body.uid }, 'Error in runOne endpoint');
            return reply.code(500).send({
