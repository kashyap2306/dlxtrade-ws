...
async function chatbotRoutes(fastify) {
    ...
    fastify.get('/chatbot/test', async (request, reply) => {
        return { status: 'ok', message: 'Chatbot route is working' };
    });
    // OPTIONS /api/chatbot - CORS preflight
    fastify.options('/chatbot', async (request, reply) => {
        try {
            ...
            logger_1.logger.error({ err }, 'Error in chatbot OPTIONS handler');
    ...
    fastify.post('/chatbot', {
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.error({ err }, 'Error accessing user from request');
            ...
            if (!user || !user.uid) {
                logger_1.logger.warn('Chatbot request without valid user');
            ...
            try {
                ...
                if (!geminiApiKey || geminiApiKey.trim() === '') {
                    logger_1.logger.error({ uid: user.uid, hasEnvKey: !!process.env.GEMINI_API_KEY }, 'GEMINI_API_KEY not configured');
                    return reply.code(500).send({
...
Be concise, friendly, and professional. If asked about specific trading strategies or financial advice, remind users to do their own research.`;
                ...
                try {
                    ...
                    logger_1.logger.info({ uid: user.uid }, 'Attempting to use Gemini SDK');
                    const geminiModule = await Promise.resolve().then(() => __importStar(require('@google/generative-ai')));
                    const GoogleGenerativeAI = geminiModule?.GoogleGenerativeAI || geminiModule?.default?.GoogleGenerativeAI;
                    if (GoogleGenerativeAI) {
                        const genAI = new GoogleGenerativeAI(geminiApiKey);
                        const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
                        logger_1.logger.info({ uid: user.uid, messageLength: body.message.length }, 'Calling Gemini API via SDK');
                        ...
                        logger_1.logger.info({ uid: user.uid, responseLength: text.length }, 'Gemini API call successful via SDK');
                    }
                    else {
                    ...
                    try {
                        logger_1.logger.info({ uid: user.uid, messageLength: body.message.length }, 'Calling Gemini API via HTTP');
                        text = await callGeminiAPIHttp(geminiApiKey, fullPrompt);
                        logger_1.logger.info({ uid: user.uid, responseLength: text.length }, 'Gemini API call successful via HTTP');
                    }
                    catch (httpError) {
                ...
                if (err instanceof zod_1.z.ZodError) {
                    logger_1.logger.warn({ errors: err.errors, uid: user?.uid }, 'Chatbot request validation failed');
                    return reply.code(400).send({
