    ...
    try {
        ...
        if (envRaw) {
            try {
                ...
                logger_1.logger.warn({ err }, 'Failed to parse FIREBASE_SERVICE_ACCOUNT from env, using default');
        ...
        logger_1.logger.info({ projectId }, 'Firebase Admin initialized successfully');
    }
    catch (error) {
        console.error('❌ Error initializing Firebase Admin:', error.message);
        logger_1.logger.error({ error: error.message, stack: error.stack }, 'Error initializing Firebase Admin');
