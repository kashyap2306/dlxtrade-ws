...
async function ensureUser(uid, profileData) {
    ...
    try {
        ...
        if (!existingUser.exists) {
            ...
            createdNew = true;
            logger_1.logger.info({ uid, createdNew: true, path: `users/${uid}` }, '✅ Main user document created: users/{uid}');
        }
        else {
            ...
            if (Object.keys(updateData).length > 2) { // More than just updatedAt and lastLogin
                ...
                logger_1.logger.info({ uid, updatedFields: Object.keys(updateData) }, 'User document updated with missing fields');
            }
            else {
                await userRef.update(updateData); // Still update lastLogin
            }
            logger_1.logger.info({ uid, createdNew: false, path: `users/${uid}` }, '✅ Main user document exists: users/{uid}');
        }
        ...
        if (!existingEngineStatus.exists) {
            ...
            logger_1.logger.info({ uid }, 'Engine status document created');
        }
        ...
        if (!existingPrefs.exists) {
            ...
            logger_1.logger.info({ uid }, 'UI preferences document created');
        }
        ...
        if (!existingSettings.exists) {
            ...
            logger_1.logger.info({ uid }, 'Settings document created');
        }
        ...
        if (!existingProfile.exists) {
            ...
            logger_1.logger.info({ uid }, 'User profile document created');
        }
        else {
            ...
            logger_1.logger.info({ uid }, 'User profile document updated');
        }
        ...
        if (!existingUserSettings.exists) {
            ...
            logger_1.logger.info({ uid }, 'User settings subcollection document created');
        }
        else {
            ...
            if (Object.keys(settingsUpdate).length > 0) {
                ...
                logger_1.logger.info({ uid }, 'User settings updated to match schema');
            }
        }
        ...
        if (!existingUserUIPrefs.exists) {
            ...
            logger_1.logger.info({ uid }, 'User UI preferences subcollection document created');
        }
        ...
        for (const apiName of integrations) {
            ...
            if (!doc.exists) {
                ...
                logger_1.logger.info({ uid, apiName, path: `users/${uid}/integrations/${apiName}` }, `✅ Research integration doc created: users/{uid}/integrations/${apiName}`);
            }
        }
        ...
        if (!exchangeConfigDoc.exists) {
            ...
            logger_1.logger.info({ uid, path: `users/${uid}/exchangeConfig/current` }, `✅ Trading exchange config doc created: users/{uid}/exchangeConfig/current`);
        }
        else {
            // Ensure required fields exist
            ...
            if (Object.keys(updateData).length > 0) {
                ...
                logger_1.logger.info({ uid, updatedFields: Object.keys(updateData), path: `users/${uid}/exchangeConfig/current` }, 'Exchange config updated with missing fields');
            }
        }
        ...
        const initCollection = async (path) => {
        ...
        try {
            ...
            for (const doc of allAgentsSnapshot.docs) {
                ...
                if (!userAgentDoc.exists) {
                    ...
                    const patch = {};
                    if (u.name === undefined)
                        patch.name = agentData.name || agentId;
                    if (u.description === undefined)
                        patch.description = agentData.description || '';
                    patch.updatedAt = now;
            ...
            logger_1.logger.warn({ err }, 'Could not initialize user agents subcollection (continuing)');
        }
        // 11. Log activity (USER_CREATED for new users, USER_LOGIN for existing)
        const activityType = createdNew ? 'USER_CREATED' : 'USER_LOGIN';
        await firestoreAdapter_1.firestoreAdapter.logActivity(uid, activityType, {
            message: createdNew
        ...
        try {
            ...
            logger_1.logger.error({ logError }, 'Failed to log error to Firestore');
        }
        return {
            success: false,
            createdNew: false,
...
async function onboardNewUser(uid, userData) {
