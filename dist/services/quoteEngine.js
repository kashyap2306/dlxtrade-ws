"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.quoteEngine = exports.QuoteEngine = void 0;
const logger_1 = require("../utils/logger");
const orderManager_1 = require("./orderManager");
const riskManager_1 = require("./riskManager");
class QuoteEngine {
    constructor() {
        this.adapter = null;
        this.isRunning = false;
        this.uid = null;
        this.activeQuotes = new Map();
        this.lastMidPrice = new Map();
        this.cancelTimers = new Map();
    }
    async start(config, adapter, uid) {
        if (this.isRunning) {
            throw new Error('Engine already running');
        }
        this.config = config;
        this.adapter = adapter;
        this.uid = uid;
        this.isRunning = true;
        logger_1.logger.info({ config }, 'Quote engine started');
        // Subscribe to orderbook updates
        this.adapter.subscribeOrderbook(config.symbol, (orderbook) => {
            this.handleOrderbookUpdate(orderbook);
        });
        // Start quoting loop
        this.quoteLoop();
    }
    async stop() {
        if (!this.isRunning)
            return;
        this.isRunning = false;
        // Cancel all active quotes
        for (const [symbol, quotes] of this.activeQuotes.entries()) {
            if (quotes.bidOrderId && this.uid) {
                try {
                    await orderManager_1.orderManager.cancelOrder(this.uid, quotes.bidOrderId);
                }
                catch (err) {
                    logger_1.logger.error({ err, orderId: quotes.bidOrderId }, 'Error canceling bid quote');
                }
            }
            if (quotes.askOrderId && this.uid) {
                try {
                    await orderManager_1.orderManager.cancelOrder(this.uid, quotes.askOrderId);
                }
                catch (err) {
                    logger_1.logger.error({ err, orderId: quotes.askOrderId }, 'Error canceling ask quote');
                }
            }
        }
        // Clear timers
        for (const timer of this.cancelTimers.values()) {
            clearTimeout(timer);
        }
        this.cancelTimers.clear();
        this.activeQuotes.clear();
        logger_1.logger.info('Quote engine stopped');
    }
    async quoteLoop() {
        while (this.isRunning) {
            try {
                await this.updateQuotes();
                await new Promise((resolve) => setTimeout(resolve, 100)); // 100ms loop
            }
            catch (err) {
                logger_1.logger.error({ err }, 'Error in quote loop');
                await new Promise((resolve) => setTimeout(resolve, 1000)); // Back off on error
            }
        }
    }
    async updateQuotes() {
        if (!this.adapter || !this.isRunning)
            return;
        const symbol = this.config.symbol;
        // Check risk limits
        if (!riskManager_1.riskManager.canTrade()) {
            logger_1.logger.warn('Risk manager blocked trading');
            return;
        }
        // Get current orderbook
        const orderbook = await this.adapter.getOrderbook(symbol, 20);
        const midPrice = this.calculateMidPrice(orderbook);
        if (!midPrice)
            return;
        const lastMid = this.lastMidPrice.get(symbol) || midPrice;
        this.lastMidPrice.set(symbol, midPrice);
        const activeQuote = this.activeQuotes.get(symbol);
        const now = Date.now();
        // Check for adverse selection
        if (activeQuote) {
            const priceMove = Math.abs(midPrice - lastMid) / lastMid;
            const timeSinceQuote = now - activeQuote.timestamp;
            if (priceMove > this.config.adversePct && timeSinceQuote < this.config.cancelMs) {
                // Cancel existing quotes due to adverse selection
                logger_1.logger.info({ symbol, priceMove, timeSinceQuote }, 'Adverse selection detected, canceling quotes');
                await this.cancelQuotes(symbol);
                return;
            }
        }
        // Check if we need to place new quotes
        if (!activeQuote || (now - activeQuote.timestamp) > this.config.cancelMs * 2) {
            await this.placeQuotes(orderbook, midPrice);
        }
    }
    calculateMidPrice(orderbook) {
        if (orderbook.bids.length === 0 || orderbook.asks.length === 0) {
            return null;
        }
        const bestBid = parseFloat(orderbook.bids[0].price);
        const bestAsk = parseFloat(orderbook.asks[0].price);
        return (bestBid + bestAsk) / 2;
    }
    async placeQuotes(orderbook, midPrice) {
        if (!this.adapter)
            return;
        const symbol = this.config.symbol;
        const spread = parseFloat(orderbook.asks[0].price) - parseFloat(orderbook.bids[0].price);
        const halfSpread = spread / 2;
        // Calculate quote prices
        const bidPrice = midPrice - halfSpread * 0.5; // Slightly inside the spread
        const askPrice = midPrice + halfSpread * 0.5;
        // Check inventory limits
        const position = await riskManager_1.riskManager.getPosition(symbol);
        const maxPos = this.config.maxPos;
        try {
            // Place bid quote if within position limits
            if (position < maxPos && this.uid) {
                const bidOrder = await orderManager_1.orderManager.placeOrder(this.uid, {
                    symbol,
                    side: 'BUY',
                    type: 'LIMIT',
                    quantity: this.config.quoteSize,
                    price: bidPrice,
                });
                if (bidOrder) {
                    const quote = this.activeQuotes.get(symbol) || { timestamp: Date.now() };
                    quote.bidOrderId = bidOrder.id;
                    this.activeQuotes.set(symbol, quote);
                    const timer = setTimeout(() => {
                        this.cancelQuotes(symbol);
                    }, this.config.cancelMs);
                    this.cancelTimers.set(`${symbol}-bid`, timer);
                }
            }
            // Place ask quote if within position limits
            if (position > -maxPos && this.uid) {
                const askOrder = await orderManager_1.orderManager.placeOrder(this.uid, {
                    symbol,
                    side: 'SELL',
                    type: 'LIMIT',
                    quantity: this.config.quoteSize,
                    price: askPrice,
                });
                if (askOrder) {
                    const quote = this.activeQuotes.get(symbol) || { timestamp: Date.now() };
                    quote.askOrderId = askOrder.id;
                    this.activeQuotes.set(symbol, quote);
                    // Set cancel timer
                    const timer = setTimeout(() => {
                        this.cancelQuotes(symbol);
                    }, this.config.cancelMs);
                    this.cancelTimers.set(`${symbol}-ask`, timer);
                }
            }
        }
        catch (err) {
            logger_1.logger.error({ err }, 'Error placing quotes');
        }
    }
    async cancelQuotes(symbol) {
        const quote = this.activeQuotes.get(symbol);
        if (!quote)
            return;
        try {
            if (quote.bidOrderId && this.uid) {
                await orderManager_1.orderManager.cancelOrder(this.uid, quote.bidOrderId);
                const timer = this.cancelTimers.get(`${symbol}-bid`);
                if (timer) {
                    clearTimeout(timer);
                    this.cancelTimers.delete(`${symbol}-bid`);
                }
            }
            if (quote.askOrderId && this.uid) {
                await orderManager_1.orderManager.cancelOrder(this.uid, quote.askOrderId);
                const timer = this.cancelTimers.get(`${symbol}-ask`);
                if (timer) {
                    clearTimeout(timer);
                    this.cancelTimers.delete(`${symbol}-ask`);
                }
            }
        }
        catch (err) {
            logger_1.logger.error({ err }, 'Error canceling quotes');
        }
        this.activeQuotes.delete(symbol);
    }
    handleOrderbookUpdate(orderbook) {
        // Update quotes if mid-price moved significantly
        const midPrice = this.calculateMidPrice(orderbook);
        if (!midPrice)
            return;
        const symbol = orderbook.symbol;
        const lastMid = this.lastMidPrice.get(symbol);
        if (lastMid) {
            const priceMove = Math.abs(midPrice - lastMid) / lastMid;
            if (priceMove > this.config.adversePct) {
                this.cancelQuotes(symbol);
            }
        }
    }
    getStatus() {
        return {
            running: this.isRunning,
            config: this.config || null,
        };
    }
}
exports.QuoteEngine = QuoteEngine;
exports.quoteEngine = new QuoteEngine();
