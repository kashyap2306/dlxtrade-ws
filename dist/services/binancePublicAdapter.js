...
exports.BinancePublicAdapter = void 0;
...
class BinancePublicAdapter {
    constructor(baseUrl = 'https://api.binance.com') {
        ...
        this.futuresHttpClient = axios_1.default.create({
            baseURL: 'https://fapi.binance.com',
            timeout: 10000,
        });
        this.initializeSymbolData();
    }
    async initializeSymbolData() {
        try {
            ...
            const symbols = response.data.symbols.map((s) => s.symbol.toUpperCase());
            this.binanceSymbols = new Set(symbols);
            logger_1.logger.debug({ symbolCount: symbols.length }, 'Binance symbols loaded successfully');
        }
        catch (error) {
            logger_1.logger.warn({ error: error.message }, 'Failed to load Binance symbols, will proceed without validation');
    ...
    async getOrderbook(symbol, limit = 20) {
        ...
        try {
            ...
            apiUsageTracker_1.apiUsageTracker.increment('binance');
            ...
            logger_1.logger.warn({ symbol, finalSymbol, error: error.message }, '[BinancePublicAdapter] getOrderbook failed, using fallback');
            return {
                symbol: finalSymbol,
                bids: [],
                asks: [],
    ...
    async getTicker(symbol) {
        if (symbol) {
            ...
            try {
                ...
                logger_1.logger.warn({ symbol, finalSymbol, error: error.message }, '[BinancePublicAdapter] getTicker failed, using fallback');
                return {
                    lastPrice: 0,
                    volume: 0,
        ...
        try {
            ...
            logger_1.logger.warn({ error: error.message }, '[BinancePublicAdapter] getAllTickers failed, using fallback');
            return [];
        }
    }
    async getBookTicker(symbol) {
        ...
        try {
            ...
            logger_1.logger.warn({ symbol, finalSymbol, error: error.message }, '[BinancePublicAdapter] getBookTicker failed, using fallback');
            return {
                symbol: finalSymbol,
                bidPrice: 0,
                askPrice: 0,
    ...
    async getKlines(symbol, interval = '1m', limit = 100) {
        ...
        try {
            ...
            logger_1.logger.warn({ symbol, finalSymbol, error: error.message }, '[BinancePublicAdapter] getKlines failed, using fallback');
            return [{
                    time: Date.now(),
                    open: 0,
                    high: 0,
    ...
    async getVolatility(symbol) {
        ...
        try {
            ...
            logger_1.logger.warn({ symbol, error: error.message }, '[BinancePublicAdapter] getVolatility failed, using fallback');
            return 0.05; // Fallback volatility (5%)
        }
    }
    async getDerivativesData(symbol) {
        ...
        try {
            ...
            apiUsageTracker_1.apiUsageTracker.increment('binance_futures');
            if (fundingResponse.status !== 200) {
                logger_1.logger.warn({ symbol: finalSymbol, status: fundingResponse.status, response: fundingResponse.data }, 'Binance futures funding rate API returned non-200');
            }
            ...
            if (oiResponse.status !== 200) {
                logger_1.logger.warn({ symbol: finalSymbol, status: oiResponse.status, response: oiResponse.data }, 'Binance futures open interest API returned non-200');
            }
            ...
            try {
                ...
                if (lsrResponse.status !== 200) {
                    logger_1.logger.warn({ symbol: finalSymbol, status: lsrResponse.status, response: lsrResponse.data }, 'Binance futures long/short ratio API returned non-200');
                }
                ...
                logger_1.logger.warn({ symbol: finalSymbol, error: lsrError.message }, 'Binance futures long/short ratio API failed (endpoint may not be available)');
            }
            return {
                fundingRate,
                openInterest,
    ...
    async getRVOL(symbol, lookbackDays = 7) {
        const finalSymbol = this.normalizeSymbol(symbol);
        try {
            ...
            logger_1.logger.warn({ symbol, finalSymbol, error: error.message }, '[BinancePublicAdapter] getRVOL failed, using fallback');
            return { rvol: 1.0, isConfirmed: false, avgVolume: 0 };
        }
    }
...
exports.BinancePublicAdapter = BinancePublicAdapter;
