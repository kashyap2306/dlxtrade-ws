...
exports.TradingEngine = exports.TradingEngineError = exports.tradingEngine = void 0;
...
exports.TradingEngineError = TradingEngineError;
class TradingEngine {
    constructor() {
        this.buckets = new Map();
        this.balanceCache = new Map();
    }
    getBucket(key, limitPerMinute) {
    ...
    consumeToken(key, limitPerMinute) {
    ...
    normalizePositions(raw, symbolFilter) {
        if (!Array.isArray(raw))
            return [];
        return raw
            .map((pos) => {
            ...
            const side = rawSide.includes('SHORT') ? 'SHORT' : 'LONG';
            return {
                symbol,
                side,
    ...
    async getBalance(uid, { forceRefresh = false } = {}) {
        ...
        if (typeof context.adapter.getBalance === 'function') {
            ...
            this.balanceCache.set(cacheKey, { value: summary, expires: Date.now() + 10000 });
            return summary;
        }
    ...
    async placeMarketOrder(uid, symbol, side, size, options = {}) {
        if (size <= 0 || !isFinite(size)) {
            throw new TradingEngineError('Order size must be greater than zero');
        }
        const context = await this.requireContext(uid);
        this.consumeToken(`${uid}:${context.name}:orders`, options.rateLimitPerMinute ?? 30);
    ...
    async closePosition(uid, symbol) {
