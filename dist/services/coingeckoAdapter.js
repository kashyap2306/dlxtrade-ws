...
const coinGeckoAdapter = {
     ...
     */
    getHistoricalData: async function (symbol, days = 90) {
        this.initialize();
        // Map symbol to CoinGecko format (bitcoin for BTC, ethereum for ETH, etc.)
        const coinId = this.mapSymbolToCoinId(symbol);
        if (!coinId) {
            logger_1.logger.warn({ symbol }, 'Could not map symbol to CoinGecko coin ID, using fallback data');
        ...
        try {
            ...
            logger_1.logger.debug({ symbol, coinId, count: historicalData.length }, 'CoinGecko historical data retrieved successfully');
            ...
            if (status === 429) {
                ...
                try {
                    ...
                    const historicalData = retryData.prices.map((pricePoint, index) => {
     ...
     */
    generateFallbackHistoricalData: function (symbol, days) {
        const now = Date.now();
        ...
        logger_1.logger.debug({ symbol, days, count: historicalData.length }, 'Generated fallback historical data');
        return { historicalData };
    },
    /**
     ...
     */
    mapSymbolToCoinId: function (symbol) {
        const symbolMap = {
            'BTC': 'bitcoin',
            'BTCUSDT': 'bitcoin',
            'ETH': 'ethereum',
