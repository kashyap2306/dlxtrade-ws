// Direct Deep Research test using service-level API keys (bypassing Firebase)
const { deepResearchEngine } = require('./dlxtrade-ws/dist/services/deepResearchEngine');

async function testDirectResearch() {
  console.log('🔬 DIRECT DEEP RESEARCH DIAGNOSTIC TEST');
  console.log('='.repeat(60));
  console.log('Using service-level API keys (no Firebase required)');
  console.log('');
  ...
  try {
    ...
    console.log(`📊 Testing research for BTCUSDT with user: ${testUserId}`);

    const startTime = Date.now();

    // Call the deepResearch function directly with service-level keys
    ...
    if (successfulProviders === 0) {
      ...
      console.log('🔧 Check: Service-level API keys, network connectivity, provider service status');
