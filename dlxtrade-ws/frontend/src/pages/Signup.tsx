...
export default function Signup() {
  ...
  const handleAfterSignUp = async (userCredential: any, phoneNumber?: string) => {
    try {
      ...
      // Update user document with phone if provided
      if (phoneNumber) {
        try {
          const userRef = doc(db, 'users', userCredential.user.uid);
          await userRef.set({ phone: phoneNumber }, { merge: true });
        } catch (err) {
          suppressConsoleError(err, 'updatePhone');
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await handleAfterSignUp(userCredential, phone);
    } catch (err: any) {
