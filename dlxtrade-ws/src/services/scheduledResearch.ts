...
/**
 * ⚠️ DEPRECATED: Scheduled Research Service
 * 
 * THIS SERVICE IS DEPRECATED AND DISABLED.
 * 
 * Use BackgroundResearchScheduler instead, which provides:
 * - User-specific frequency control
 * - Mode-based execution (Auto-Trade vs Telegram)
 * - Unified research execution
 * - Proper state management
 * 
 * This file is kept for reference only and should NOT be started.
 * It is commented out in server.ts and will NOT run.
 * 
 * Original purpose:
 * - Ran deep research every 5 minutes for all active users
 * - Used ONLY research APIs: CryptoCompare, NewsData, CoinMarketCap
 * 
 * STRICTLY FORBIDDEN:
...
export class ScheduledResearchService {
   ...
   * 
   * REFACTORED: Uses safe background runner for event loop protection
   */
  start() {
    try {
      // Check env flag first
      if (process.env.DISABLE_AUTOTRADE === 'true') {
        logger.warn('Scheduled research service DISABLED by env flag');
        console.log('🛑 [SCHEDULED_RESEARCH] Service DISABLED by DISABLE_AUTOTRADE=true');
        return;
      }
      ...
      // REFACTORED: Use runBackgroundTask for initial run (non-blocking)
      runBackgroundTask(
        () => this.runScheduledResearchSafe(),
        'initial-scheduled-research',
        30000 // 30s timeout
      ).catch((err) => {
        logger.error({ error: err?.message }, 'Error in initial scheduled research run');
      });
      
      // REFACTORED: Use safeSetInterval for event loop protection
      this.intervalId = safeSetInterval(
        async () => {
          await this.runScheduledResearchSafe();
        },
        5 * 60 * 1000, // 5 minutes
        'scheduled-research'
      );
  ...
  /**
   * Safe wrapper for runScheduledResearch
   * Adds timeout and event loop protection
   */
  private async runScheduledResearchSafe(): Promise<void> {
    // Check if background tasks should run
    if (!shouldRunBackgroundTasks()) {
      logger.debug('Skipping scheduled research - background tasks paused');
      return;
    }
    ...
    try {
      await withTimeout(
        ...
        60000, // 60s timeout for full scheduled research
        'scheduled-research-cycle'
      );
    } catch (err: any) {
      logger.warn({ error: err.message }, 'Scheduled research cycle timed out or failed');
    }
  }
  ...
  async runResearchForUser(uid: string): Promise<{
    ...
    logger.info({ uid }, '[BACKGROUND_STARTED]');

    // CHECK COOLDOWN: Prevent background research overload
    try {
      const db = getFirebaseAdmin().firestore();
      const userDoc = await db.collection('users').doc(uid).get();

      if (userDoc.exists) {
        const lastBackgroundResearchAt = userDoc.data()?.lastBackgroundResearchAt;
        if (lastBackgroundResearchAt) {
          const lastRunTime = lastBackgroundResearchAt.toDate().getTime();
          const now = Date.now();
          ...
          if (timeSinceLastRun < cooldownMs) {
            ...
            logger.warn({ uid, remainingMinutes }, '[BACKGROUND_SKIPPED_COOLDOWN]');
            return {
              success: false,
              symbol: 'BTCUSDT',
              errors: [{
                adapter: 'Cooldown',
                error: `Background research skipped due to cooldown. Try again in ${remainingMinutes} minutes.`,
                isAuthError: false,
              }],
            };
          }
      ...
      logger.warn({ uid, error: cooldownError.message }, '[BACKGROUND_ABORTED_COOLDOWN_CHECK]');
      // Continue with research even if cooldown check fails
    }
    ...
    try {
      ...
      try {
        ...
        await db.collection('users').doc(uid).update({
          lastBackgroundResearchAt: admin.firestore.Timestamp.now(),
        });
        logger.info({ uid }, '[BACKGROUND_COMPLETED]');
      } catch (updateError: any) {
        logger.warn({ uid, error: updateError.message }, 'Failed to update lastBackgroundResearchAt timestamp');
      }
      ...
      logger.error({ error: error.message, uid }, '[BACKGROUND_ABORTED_TIMEOUT]');
