...
export class AgentExecutionService {
  ...
  async loadActiveAgents(): Promise<void> {
    try {
      console.log('[LOAD_ACTIVE_AGENTS] Fetching active agents from Firestore...');
      ...
      for (const cfg of agentConfigs) {
        ...
        // Deduplicate to ONE HTF agent per user to avoid double-execution of the multi-pair logic
        // The strategy executes for ALL allowed pairs regardless of the configured pair
        const key = `${userId}|HTF_TREND_FILTER`;
      ...
      console.error('[LOAD_ACTIVE_AGENTS] ERROR:', error);
      logger.error({
   ...
   */
  async executeAllAgents(tickTime: number = Date.now()): Promise<void> {
    ...
    // ONE-SHOT FIX: Update lastSchedulerCycleId when cycle changes
    if (schedulerCycleId !== AgentExecutionService.lastSchedulerCycleId) {
      ...
      logger.info({ schedulerCycleId }, 'New scheduler cycle started');
    }
    ...
    for (const agent of this.activeAgents.values()) {
      ...
      if (isHTFAgent) {
        ...
        tradingAgentPromises.push(this.executeAgent(agent, schedulerCycleId, undefined, undefined, 'background_job', 'scheduler', tickTime));
      }
    }

    // Execute VWAP strategy agents
    const vwapStrategyPromises = Array.from(this.activeVWAPStrategies.entries())
      ...
      .map(([userId, strategy]) => strategy.execute(this.marketDataProvider));
  ...
  private async executeAgent(
    ...
    source?: 'background_job' | 'scheduler' | 'ui',
    tickTime?: number
  ): Promise<void> {
    const agentId = agent['config'].id;
    const agentConfig = agent['config'];
    // FIX: Define triggerType early to avoid ReferenceError in trace logs
    const triggerType = context === 'background_job' ? 'scheduler' : 'manual';
    ...
    if (isHTFAgent && source !== 'scheduler') {
      console.log("[HTF_EXECUTION_BLOCKED] source=" + source + " context=" + context);
      logger.info({ agentId, source, context }, "[HTF_EXECUTION_BLOCKED] HTF execution blocked from non-scheduler source");
      return; // RETURN IMMEDIATELY - No logic runs, NO diagnostics written
    }
    ...
    // FIXED 5-MINUTE BUCKET CYCLE ID (LINKED TO TICK TIME)
    const effectiveTickTime = tickTime || Date.now();
    ...
    let bucketGuardPassed = false; // Guard to track if query-based bucket check passed

    // ATOMIC WRITE-TIME BUCKET LOCK (PAIR-AWARE): Prevent duplicate HTF executions across any overlap
    // This provides a hard atomic guarantee even across multiple instances or concurrent calls
    if (isHTFAgent) {
      const userId = agentConfig.userId;
      if (!userId) {
        logger.warn({ agentId }, '[HTF_ATOMIC_LOCK] No userId found - skipping lock');
        // If no userId, we can't lock, but we shouldn't allow the writer either without identity
        return;
      } else {
        try {
          const db = getFirebaseAdmin().firestore();
          ...
          if (!wasLockAcquired) {
            logger.info({ agentId, userId, symbol, bucketStartMs }, '[HTF_ATOMIC_LOCK] Lock already exists for this 5-minute bucket and symbol - skipping execution');
            return; // EXIT: Another process/instance already acquired the lock
          }

          logger.debug({ agentId, lockDocPath }, '[HTF_ATOMIC_LOCK] Lock acquired successfully');

          // 5. HTF_BUCKET_GUARD: Secondary query-based check for existing diagnostics
          // This ensures that even if we have the lock doc, we don't proceed if a diagnostic exists
          // CRITICAL: Change from fail-open to FAIL-CLOSE as per requirements
          try {
            const entriesRef = db.collection('users').doc(userId).collection('agentDiagnostics').doc(agentId).collection('entries');

            // This query requires a composite index on (tradingPair, runtimeState.bucketStartMs)
            const existing = await entriesRef
              .where('tradingPair', '==', tradingPair)
              .where('runtimeState.bucketStartMs', '==', bucketStartMs)
              .limit(1)
              .get();

            if (!existing.empty) {
              logger.info({ agentId, userId, tradingPair, bucketStartMs }, '[HTF_BUCKET_GUARD] Diagnostic already exists for this bucket - skipping execution');
              return; // Already executed and saved
            }

            // Mark as passed only if query successfully verified no existing diagnostic
            bucketGuardPassed = true;
          } catch (guardError: any) {
            // FAIL-CLOSE: If the query fails (missing index/FAILED_PRECONDITION), we stop
            logger.error({
              agentId,
              error: guardError.message,
              code: guardError.code
            }, '[HTF_BUCKET_GUARD] Guard failed (likely missing index) - FAIL CLOSED');
            return; // EXIT IMMEDIATELY - No logic runs, NO diagnostics written
          }
        } catch (lockError) {
          logger.error({
            agentId,
            error: lockError instanceof Error ? lockError.message : String(lockError)
          }, '[HTF_ATOMIC_LOCK] Transaction failed while acquiring lock - skipping for safety');
          return; // Skip on error to ensure we never allow duplicates under contention
        }
      }
    }
    ...
    const cycleId = isHTFAgent ? htfCycleId : (pairCycleId || schedulerCycleId || (() => {
    ...
    // CRITICAL HTF FIX: Wrap execution in try-finally to ALWAYS write diagnostics for HTF agents
    // This ensures diagnostics are written every 5-minute cycle regardless of early returns
    try {
      ...
      try {
        ...
        if (!currentAgentConfig || currentAgentConfig.status === 'STOPPED') {
          ...
          // CRITICAL HTF FIX: Populate required diagnostic fields BEFORE early return
          if (isHTFAgent) {
            diagnostics.tradingPair = tradingPair;
        ...
        if (isBBRsiAgent) {
          ...
          if (htfTrend.direction === 'NO_TRADE') {
            ...
            // CRITICAL: skipReasonShort will be derived from htfTrend.reason in finally block
            diagnostics.executionState.researchedOnly = true;
            diagnostics.executionState.executionEvaluated = true;
          ...
          if (!ltfSignal.isValid) {
            ...
            // CRITICAL: Generate skipReasonShort (3-4 words) from FIRST rejected indicator
            let skipReasonShort = 'Conditions not met';
            if (ltfSignal.indicators?.results) {
              const results = ltfSignal.indicators.results;
      ...
      } // Close inner try-catch (execution logic)
    } // Close outer try block
    catch (outerError) {
      // Catch any errors from the entire execution flow (outer try-catch)
      logger.error({
        agentId,
        tradingPair,
        error: outerError instanceof Error ? outerError.message : 'Unknown error'
      }, 'Unexpected error in agent execution flow');

      // Ensure diagnostics reflect the error
      if (!diagnostics.decision || diagnostics.decision.action === 'SKIP') {
        diagnostics.decision = {
          action: 'SKIP',
          reason: outerError instanceof Error ? outerError.message : 'Unexpected execution error'
        };
      }
    } finally {
      // CRITICAL HTF FIX: ALWAYS write diagnostics for HTF agents in finally block
      // This ensures diagnostics are written EVERY 5-minute cycle regardless of early returns
      // The finally block executes even when there are return statements in the try block
      if (isHTFAgent) {
        // HARD GUARD: Only write HTF diagnostics if BOTH the atomic lock and bucket guard passed
        // This prevents threads that lost the atomic race or encountered guard errors from writing "skipped" diagnostics in finally
        if (!wasLockAcquired || !bucketGuardPassed) {
          logger.debug({
            agentId: agentId,
            pair: tradingPair,
            bucketStartMs,
            wasLockAcquired,
            bucketGuardPassed
          }, 'HTF diagnostic write SKIPPED - lock not acquired or bucket guard failed');
          return;
        }
        ...
        try {
          ...
          const canonicalIndicators: any = {
            ...
            trend: {
              ...
              reason: htfTrend?.reason || 'Trend alignment failed'
            }
          };

          // Persist in runtimeState as requested
          if (!diagnostics.runtimeState) diagnostics.runtimeState = {};
          diagnostics.runtimeState.indicators = canonicalIndicators;

          // Mirror top-level flags for UI safety
          diagnostics.runtimeState.emaConfirmed = canonicalIndicators.ema.confirmed;
          diagnostics.runtimeState.rsiConfirmed = canonicalIndicators.rsi.confirmed;
          diagnostics.runtimeState.vwapConfirmed = canonicalIndicators.vwap.confirmed;
          diagnostics.runtimeState.srConfirmed = canonicalIndicators.sr.confirmed;
          diagnostics.runtimeState.volumeConfirmed = canonicalIndicators.volume.confirmed;
          ...
          await firestoreAdapter.saveAgentDiagnostic(agentId, {
            ...
            runtimeState: {
              ...
              cycleId: htfCycleId,
              bucketStartMs,
              signalGenerated: !!diagnostics.signal?.direction
            }
          }, diagnostics.userId || agentConfig.userId);

          // 7. Mark cycle as written to prevent duplicate writes within the same scheduler tick
          if (diagnostics.cycleId) {
            logger.debug({ agentId: agentId, cycleId: diagnostics.cycleId }, 'HTF evaluation record successfully persisted');
          }
        } catch (diagError) {
          logger.error({
            agentId,
            error: diagError instanceof Error ? diagError.message : 'Unknown error'
          }, 'CRITICAL: Failed to persist HTF agent diagnostics in finally block');
        }
      } else {
        // Non-HTF agents: write diagnostics only if execution started
        try {
  ...
  public async executeAgentManually(agentId: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      await this.executeAgent(agent, undefined, undefined, undefined, 'user_request', 'ui');
