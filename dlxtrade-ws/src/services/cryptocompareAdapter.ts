...
export class CryptoCompareAdapter {
  ...
  async getMarketData(symbol: string): Promise<any> {
    // HARD GUARANTEE: Block invalid or mis-converted symbols (Task 4)
    const upperSymbol = symbol?.toUpperCase();
    if (upperSymbol === 'MUSDT' || upperSymbol === 'MBTCUSDT' || upperSymbol === 'M' || !upperSymbol?.endsWith('USDT')) {
      logger.warn({ symbol }, "CryptoCompare blocked invalid or auto-converted symbol");
      return {
        provider: 'cryptocompare',
        success: false,
        error: `Invalid pair: ${symbol}. Only USDT perpetual pairs supported.`,
        symbol,
        hasData: false
      };
    }
  ...
  async getHistoricalOHLC(symbol: string, limit: number = 100, type: 'hour' | 'day' = 'hour'): Promise<any[]> {
    const endpoint = type === 'day' ? '/histoday' : '/histohour';
    ...
    try {
      ...
      console.warn(`CryptoCompare historical ${type} data fetch failed for ${symbol}:`, error.message);
      return [];
    }
  }

  async getOHLCData(symbol: string): Promise<any> {
    const historicalData = await this.getHistoricalOHLC(symbol, 300, 'hour'); // Get 300 hours of data for MA50/MA200

    // FIX: Ensure historicalData is actually an array before mapping
    if (!Array.isArray(historicalData) || historicalData.length === 0) {
      return {
        success: false,
        ohlc: [],
        latest: null,
        error: 'Invalid OHLC response (not an array)'
      };
    }
  ...
  async getDailyOHLCData(symbol: string): Promise<any> {
    const historicalData = await this.getHistoricalOHLC(symbol, 300, 'day');
