...
export class BackgroundResearchScheduler {
  ...
  private async updateUserResearchSchedule(uid: string, preValidated: boolean = false, preDeterminedMode?: string, preDeterminedFrequency?: number) {
    ...
    try {
      ...
      if (preValidated && preDeterminedMode && preDeterminedFrequency) {
        ...
        if (autoTradeEnabled) {
          ...
          if (!hasPrimaryAPIs) {
            ...
            logger.warn(
              { uid },
              "⚠️ [SCHEDULER] Auto-Trade primary APIs missing - scheduler continues, research will handle gracefully at runtime",
            );
          }
          ...
          logger.info(
            {
              ...
              exchangeReason: exchangeStatus.reason, // Explicit reason for diagnostics
              hasPrimaryAPIs,
          ...
          if (!hasRequiredAPIs) {
            logger.warn(
              { uid },
              "⚠️ [SCHEDULER] Telegram Background Research: Some primary APIs missing, but continuing (runtime will handle)",
            );
          ...
          return;
        } // End of fallback mode determination
      }

      // Validate frequency
      if (!finalFrequency || finalFrequency <= 0) {
        console.log(
          "🔥 [HARD_LOG] [FREQUENCY_INVALID] Invalid frequency for user:",
          uid,
          "frequency:",
          finalFrequency,
        );
        logger.warn(
          { uid, finalFrequency },
          "⏭️ [SCHEDULER] Background research DISABLED - Invalid or missing frequency",
        );
        await this.disableUserScheduler(uid);
        return;
      }
      console.log(
        "🔥 [HARD_LOG] [FREQUENCY_VALID] Frequency validated for user:",
        uid,
        "frequency:",
        finalFrequency,
      );
      ...
      // CRITICAL: Only clear interval when MODE or FREQUENCY actually changed
      // INTERVAL_CLEAR must happen ONLY when:
      // a) switching MODE (AUTO_TRADE ↔ TELEGRAM)
      // b) frequency actually changed
      // c) HARD STOP (disconnected)
      // INTERVAL_CLEAR must NOT run for:
      // - not_connected
      // - soft skip
      // - decryption failure
      // - same mode + same frequency
      const existingMode = (existingState as any)?.mode;
      const existingFrequency = (existingState as any)?.frequencyMinutes;
      const modeChanged = existingState && existingMode !== mode;
      const frequencyChanged = existingState && existingFrequency !== finalFrequency;
      const shouldClearInterval = modeChanged || frequencyChanged;

      // DEBUG: Log interval decision details
      console.log(
        "🔥 [HARD_LOG] [INTERVAL_DECISION] uid:",
        uid,
        "existingInterval:",
        !!existingInterval,
        "existingState:",
      ...
      if (shouldClearInterval && existingInterval) {
        console.log(
          "🔥 [HARD_LOG] [INTERVAL_CLEAR] Clearing existing interval for user:",
          uid,
          "MODE_CHANGED:",
          modeChanged,
          "FREQUENCY_CHANGED:",
          frequencyChanged,
          "existingMode:",
          (existingState as any)?.mode,
          "newMode:",
          mode,
          "existingFreq:",
          (existingState as any)?.frequencyMinutes,
          "newFreq:",
          finalFrequency,
        );
        clearInterval(existingInterval);
        this.userIntervals.delete(uid);
        logger.info(
          { uid, operation: "clear_interval_mode_or_frequency_changed", modeChanged, frequencyChanged },
          "🔄 [SCHEDULER] Cleared existing interval - mode or frequency changed",
        );
      ...
      logger.info(
        {
          ...
          frequency: finalFrequency,
          mode,
          intervalCleared: shouldClearInterval,
      ...
      const userInterval = safeSetInterval(
        async () => {
          console.log(
            "🔥 [HARD_LOG] [INTERVAL_TICK_USER] User interval tick for:",
            uid,
      ...
      console.log(
        "🔥 [HARD_LOG] [INTERVAL_CREATED_USER] User interval created for:",
        uid,
        "intervalId exists:",
        !!userInterval,
      );
      ...
      // Classify error for soft vs. hard failures
      const isConnectivityError =
        error.message &&
        (error.message.includes("not_connected") ||
          error.message.includes("SOFT_SKIP") ||
      ...
      logger.error(
        {
          ...
          schedulerContinues: true,
          willRetry: true,
          isSoftError: isConnectivityError,
        },
        isConnectivityError
          ? "⚠️ [SCHEDULER_SOFT_ERROR] Connectivity error in schedule update - soft skip without state reset"
          : "❌ [SCHEDULER_IMMORTAL] Error updating user research schedule - scheduler continues, will retry on next check cycle",
      );

      // For connectivity errors, ensure state is preserved
      if (isConnectivityError) {
        // Add to soft-skip cooldown to prevent thrashing
        this.softSkipUsers.set(uid, {
          until: Date.now() + 120000, // 2 minutes
          reason: "connectivity_error",
        });

        // Ensure state continues to exist
        if (!this.userJobStates.get(uid) && this.userIntervals.has(uid)) {
          this.userJobStates.set(uid, {
            isRunning: false,
            lastRunAt: null,
            nextRunAt: null,
            mode: "AUTO_TRADE_RESEARCH", // Default to AUTO_TRADE_RESEARCH
          });
        }
      }
  ...
  private async disableUserScheduler(uid: string) {
    ...
    if (existingInterval) {
      ...
      logger.info(
        {
          ...
          stateRemoved: true,
          reason:
            "User explicitly disabled auto-trade or telegram background research",
        },
      ...
      try {
        ...
        logger.info(
          { uid },
          "Scheduler state cleared - timers and in-memory state removed (user flags preserved)",
        );
      } catch (persistError: any) {
        logger.warn(
          { uid, error: persistError.message },
          "Failed to persist engineState (non-critical)",
        );
      }

      logger.info(
        { uid },
        "🛑 [SCHEDULER] Scheduler stopped for user - timers cleared, state removed (user enable flags NOT modified)",
      );
    }
  }
  ...
  async onUserSettingsChanged(uid: string) {
    logger.info(
      { uid },
      "🔄 [SCHEDULER] User settings changed, updating schedule",
    );
    try {
      await this.withUserLock(uid, () => this.updateUserResearchSchedule(uid));
    } catch (error) {
      if (error.message === "USER_OPERATION_LOCKED") {
        logger.info(
          { uid },
          "⏱️ [SCHEDULER] Skipping settings update due to concurrent operation",
        );
        return;
      }
      throw error;
  ...
  /**
   * Helper to ensure synchronized operations with lock
   */
  private async withUserLock(
    uid: string,
    operation: () => Promise<any>,
  ): Promise<any> {
    // Check if lock exists
    const lockInfo = this.userOperationLocks.get(uid);
    const now = Date.now();

    if (lockInfo && lockInfo.until > now) {
      logger.warn(
        { uid, lockedUntil: new Date(lockInfo.until).toISOString() },
        "🔒 [SCHEDULER] Operation skipped due to active lock",
      );
      throw new Error("USER_OPERATION_LOCKED");
    }

    try {
      // Set lock for 3 seconds
      this.userOperationLocks.set(uid, { until: now + 3000 });
  ...
  async forceStopUserScheduler(uid: string, reason?: string) {
    // FAIL-FAST SAFETY ASSERTION: Throw error if called with not_connected or anything other than disconnected
    if (
      reason !== "disconnected" &&
      reason !== "exchange_permanently_deleted"
    ) {
      const errorMessage = `INVARIANT_VIOLATION: '${reason || "unknown"}' attempted hard stop - only disconnected allowed`;
      logger.error({ uid, reason, invariantViolation: true }, errorMessage);
      throw new Error(errorMessage);
    }

    // HARD GUARD: Only call this method for hard stops, never for not_connected
    logger.info(
      { uid, reason },
      "🛑 [SCHEDULER] Force stopping scheduler for user - HARD STOP ONLY",
    );
    ...
    if (existingInterval) {
      ...
      console.log("🔥 [HARD_LOG] [SCHEDULER_INTERVAL_CLEARED]", {
        uid,
        reason: "force_stop_exchange_hard_stop_only",
      });
    }
    ...
    logger.info(
      { uid },
      "✅ [SCHEDULER] Force stopped scheduler for user - HARD STOP COMPLETED",
    );
  }
   ...
   */
  async ensureUserResearchScheduled(
    uid: string,
    ...
    try {
      ...
      if (isCurrentlyScheduled) {
        ...
        if (autoTradeEnabled) {
          logger.info(
            { uid },
            "🔄 [SCHEDULER] User already scheduled but auto-trade enabled - forcing immediate execution",
          );
          try {
            ...
            logger.info(
              { uid },
              "✅ [SCHEDULER] Forced immediate auto-trade research execution",
            );
            console.log(
              "🔥 [HARD_LOG] [AUTO_TRADE_IMMEDIATE_EXECUTION] Forced immediate research execution for auto-trade enable",
            );
          } catch (execError: any) {
            logger.warn(
              { uid, error: execError.message },
              "⚠️ [SCHEDULER] Failed to force immediate execution",
            );
          }
        }
        ...
        logger.info(
          { uid },
          "✅ [SCHEDULER] User already scheduled - no action needed",
        );
      ...
      logger.info(
        { uid, autoTradeEnabled, telegramBgResearchEnabled },
        "🔒 [SCHEDULER] Forcing user registration in scheduler",
      );
      try {
        await this.withUserLock(uid, () =>
          this.updateUserResearchSchedule(uid),
        );
      } catch (error) {
        if (error.message === "USER_OPERATION_LOCKED") {
          logger.info(
            { uid },
            "⏱️ [SCHEDULER] Skipping force registration due to concurrent operation",
          );
        } else {
          throw error;
        }
      ...
      if (isNowScheduled) {
        ...
        try {
          ...
          await firestoreAdapter.saveBackgroundResearchSettings(uid, {
            scheduled: true,
            nextRunAt: nextRunAt
              ? admin.firestore.Timestamp.fromDate(nextRunAt)
              : null,
            lastScheduledAt: admin.firestore.Timestamp.now(),
          });
          logger.info(
            { uid, nextRunAt: nextRunAt?.toISOString() },
            "✅ [SCHEDULER] User scheduled and state persisted to Firestore",
          );
        } catch (persistError: any) {
          logger.warn(
            { uid, error: persistError.message },
            "⚠️ [SCHEDULER] Failed to persist scheduled state (scheduler still active)",
          );
        }
        ...
        logger.error(
          { uid },
          "❌ [SCHEDULER] Registration failed - user not scheduled after updateUserResearchSchedule",
        );
        return {
          scheduled: false,
          reason: "Registration failed - check logs for details",
        };
      }
    } catch (error: any) {
      logger.error(
        { uid, error: error.message, stack: error.stack },
        "❌ [SCHEDULER] Error in ensureUserResearchScheduled",
      );
  ...
  async runTelegramBackgroundDiagnostic(uid: string): Promise<{
    ...
    try {
      ...
      const bgResearchSettings =
        await firestoreAdapter.getBackgroundResearchSettings(uid);
      const backgroundResearchEnabled =
        bgResearchSettings?.backgroundResearchEnabled === true;
      // CRITICAL: Check explicit telegramBackgroundResearchEnabled flag (declare early for use throughout function)
      const telegramBgResearchEnabled =
        bgResearchSettings?.telegramBackgroundResearchEnabled === true ||
        (bgResearchSettings?.backgroundResearchEnabled === true &&
          bgResearchSettings?.telegramBackgroundResearchEnabled !== false);

      if (!telegramBgResearchEnabled && !backgroundResearchEnabled) {
        blockers.push(
          "Telegram Background Research is not enabled in settings",
        );
      }

      const researchFrequencyMinutes =
        bgResearchSettings?.researchFrequencyMinutes;
      if (!researchFrequencyMinutes || researchFrequencyMinutes <= 0) {
        blockers.push(
          `Invalid research frequency: ${researchFrequencyMinutes} (must be > 0)`,
        );
      }
      ...
      if (accuracyTrigger) {
        ...
        if (typeof accuracyTrigger === "number") {
          ...
          typeof accuracyTrigger === "object" &&
          accuracyTrigger !== null
        ) {
          // New format: { min, max }
        ...
        if (minTrigger >= maxTrigger || minTrigger < 0 || maxTrigger > 100) {
          blockers.push(
            `Invalid accuracy trigger range: ${minTrigger}-${maxTrigger} (must be min < max, 0-100)`,
          );
        }
      }
      ...
      let telegramSetupStatus: "Connected" | "Missing" | "TestFailed" =
        "Missing";
      ...
      try {
        const { getUserIntegrationsByUid } =
          await import("../routes/users/providerConfig");
        const providerConfig = await getUserIntegrationsByUid(
          uid,
          "background_job",
        );
        ...
        const findProviderFamily = (
          baseName: string,
        ): { provider: any; bucket: string; key: string } | null => {
          ...
          for (const bucket of allBuckets) {
            ...
            for (const [key, provider] of Object.entries(bucketProviders)) {
              ...
              if (
                ...
                normalizedKey.startsWith(normalizedBase + "_")
              ) {
        ...
        const hasValidNewsProvider =
          providerConfig?.news &&
          Object.values(providerConfig.news).some(
            (p: any) =>
              p?.enabled === true &&
              p?.apiKey &&
        ...
        logger.warn(
          { uid, error: apiError.message },
          "Failed to load provider config for diagnostics",
        );
      ...
      let mode: "TELEGRAM_BACKGROUND_RESEARCH" | "AUTO_TRADE_RESEARCH" | null =
        null;
      if (autoTradeEnabled) {
        mode = "AUTO_TRADE_RESEARCH";
        if (telegramBgResearchEnabled) {
          blockers.push(
            "Auto-Trade mode is enabled - Telegram Background Research is automatically disabled (Auto-Trade has priority)",
          );
        }
      } else if (telegramBgResearchEnabled) {
        mode = "TELEGRAM_BACKGROUND_RESEARCH";
      ...
      if (telegramBgResearchEnabled || autoTradeEnabled) {
        if (!userScheduled) {
          blockers.push(
            "User is not registered in background research scheduler",
          );
      ...
      if (jobState) {
        if (jobState.nextRunAt) {
          ...
          logger.debug(
            { uid },
            "User scheduled but nextRunAt not yet set (will be set on first run)",
          );
        }
      } else if (
        (telegramBgResearchEnabled || autoTradeEnabled) &&
        !userScheduled
      ) {
        blockers.push(
          "Scheduler job state not found for user - user not registered",
        );
      }
      ...
      if (jobState?.lastRunAt) {
        ...
        if (lastRunTime > now) {
          blockers.push(
            `Invalid lastRunAt: ${jobState.lastRunAt.toISOString()} (in the future)`,
          );
        }
      }
      ...
      const persistedEngineState = bgResearchSettings?.engineState as
        | "RUNNING"
        | "STOPPED"
        | undefined;
      ...
      if (persistedEngineState === "RUNNING") {
        ...
        telegramBgResearchEnabled &&
        (userScheduled || jobState?.nextRunAt)
      ) {
        // Telegram Background Research enabled and scheduler active - infer RUNNING
        engineState = "RUNNING";
        logger.info(
          { uid },
          "Engine state inferred from scheduler activity and telegramBackgroundResearchEnabled",
        );
        ...
        logger.debug(
          { uid },
          "Engine state: STOPPED (background research and telegram background research disabled)",
        );
      } else {
        ...
        logger.debug(
          { uid },
          "Engine state: STOPPED (scheduler not initialized)",
        );
      }
      ...
      const hasRunCycle =
        jobState?.lastRunAt !== null && jobState?.lastRunAt !== undefined;
      ...
      return {
        ...
        lastRunAt: jobState?.lastRunAt
          ? jobState.lastRunAt.toISOString()
          : null,
        nextRunAt: jobState?.nextRunAt
          ? jobState.nextRunAt.toISOString()
          : null,
        engineState,
      ...
      logger.error(
        { uid, error: error.message, stack: error.stack },
        "Error running Telegram background diagnostic",
      );
  ...
  private async processUserResearchSafe(uid: string): Promise<void> {
    console.log(
      "🔥 [HARD_LOG] [PROCESS_SAFE_START] processUserResearchSafe() called for user:",
      uid,
    );
    ...
    try {
      console.log(
        "🔥 [HARD_LOG] [PROCESS_SAFE_EXECUTING] Calling processUserResearch() for user:",
        uid,
      );
      await withTimeout(
      ...
      console.log(
        "🔥 [HARD_LOG] [PROCESS_SAFE_COMPLETE] processUserResearch() completed for user:",
        uid,
      );
    } catch (err: any) {
      ...
      logger.warn(
        ...
        isTimeout
          ? "🕒 [SCHEDULER_TIMEOUT] User research timed out - ensuring state is properly reset before next cycle"
          : "⚠️ [SCHEDULER_IMMORTAL] User research failed - scheduler continues, will retry on next interval",
      );
      ...
      try {
        ...
        const mode =
          (jobState as any)?.mode || RESEARCH_MODE.TELEGRAM_BACKGROUND_RESEARCH; // Safe fallback
        ...
        if (jobState) {
          // CRITICAL: Always reset running state to prevent stuck jobs
          ...
          logger.info(
            ...
            "🔄 [SCHEDULER] Reset job state after error/timeout",
          );
        }
        ...
        logger.warn(
          ...
          "Failed to update state after safe wrapper error",
        );
      }

      // For timeouts, check if we need to add to soft-skip cooldown
      if (err?.message === "TIMEOUT") {
        // Add a short soft-skip cooldown to allow system recovery
        this.softSkipUsers.set(uid, {
          until: Date.now() + 60000, // 1 minute cooldown after timeout
          reason: "timeout_recovery",
        });

        logger.info(
          { uid, timeoutRecovery: true },
          "🕒 [SCHEDULER_TIMEOUT_RECOVERY] Added 1-minute soft-skip after timeout to prevent immediate retry",
        );
      }
  ...
  private async processUserResearch(uid: string) {
    ...
    console.log(
      "🔥 [HARD_LOG] [PROCESS_JOB_STATE] Job state for user:",
      uid,
      "exists:",
      !!jobState,
      "isRunning:",
    ...
    if (jobState?.isRunning) {
      logger.warn(
        { uid },
        "⏭️ [BACKGROUND_RESEARCH_SKIPPED] Background research already running for user - blocking duplicate execution",
      );
      console.log(
        "🔥 [HARD_LOG] [PROCESS_BLOCKED_DUPLICATE] Research already running, skipping for user:",
        uid,
      );
    ...
    try {
      ...
      if (this.isSystemUid(uid)) {
        logger.warn(
          { uid },
          "⚠️ [RESEARCH] System UID detected, aborting research",
        );
        ...
        try {
          ...
          logger.warn(
            ...
            "Failed to update state for system UID",
          );
        }
      ...
      console.log(
        "🔥 [HARD_LOG] [PROCESS_MODE] Mode from job state for user:",
        uid,
        "mode:",
        mode,
      );
      ...
      logger.info(
        { uid, mode },
        "🔄 [DELEGATION] Calling AutoTradeEngine for research cycle",
      );
      console.log(
        "🔥 [HARD_LOG] [RESEARCH_DELEGATE] Delegating to AutoTradeEngine for user:",
        uid,
        "mode:",
        mode,
      );
      ...
      console.log(
        "🔥 [HARD_LOG] [DECRYPTION_CHECK] Checking decryption failure cache for user:",
        uid,
        "hasCache:",
        !!failureCache,
        "disabled:",
        failureCache?.disabled,
      );
      if (failureCache?.disabled) {
        const timeSinceFailure =
          Date.now() - failureCache.lastFailureTime.getTime();
        ...
        if (timeSinceFailure < cooldownMinutes * 60 * 1000) {
          ...
          console.log(
            "🔥 [HARD_LOG] [DECRYPTION_BLOCKED] Research blocked by decryption failure cooldown for user:",
            uid,
          );
          return;
        } else {
          ...
          logger.info(
            { uid, cooldownMinutes },
            "✅ [DECRYPTION_FAILURE_GUARD] Cooldown expired, clearing cache and retrying",
          );
          console.log(
            "🔥 [HARD_LOG] [DECRYPTION_COOLDOWN_EXPIRED] Cooldown expired, clearing cache for user:",
            uid,
          );
          this.decryptionFailureCache.delete(uid);
        }
      ...
      const skipHistoryStorage =
        mode === RESEARCH_MODE.TELEGRAM_BACKGROUND_RESEARCH;
      console.log(
        "🔥 [HARD_LOG] [RESEARCH_EXEC_START] Starting research execution for user:",
        uid,
        "skipHistoryStorage:",
        skipHistoryStorage,
      );
      ...
      console.log(
        "🔥 [HARD_LOG] [SCHEDULER_CYCLE_ID] Generated cycleId:",
        schedulerCycleId,
      );
      ...
      logger.info(
        { uid, mode },
        "🔬 [RESEARCH_PRODUCER] Scheduler generating research...",
      );
      ...
      if (backgroundTasksPaused && !isAutoTradeResearch) {
        ...
        logger.info(
          { uid, mode, reason: "BACKGROUND_TASKS_PAUSED" },
          "⏸️ [SCHEDULER] Research generation skipped - background tasks paused",
        );
        console.log(
          "🔥 [HARD_LOG] [SCHEDULER_PAUSED] Background tasks paused, skipping research generation for user:",
          uid,
        );
        ...
        if (backgroundTasksPaused && isAutoTradeResearch) {
          logger.info(
            { uid, mode },
            "⚠️ [SCHEDULER] Background tasks paused but allowing AUTO_TRADE_RESEARCH to proceed",
          );
          console.log(
            "🔥 [HARD_LOG] [SCHEDULER_AUTO_TRADE_BYPASS] Background tasks paused but proceeding with auto-trade research",
          );
        }
        ...
        try {
          ...
          if (
            ...
            logger.warn(
              { uid },
              "⚠️ [RESEARCH_PRODUCER] No suitable research result found",
            );
          }
          ...
          logger.error(
            ...
            "❌ [RESEARCH_PRODUCER] Failed to generate research",
          );
        }
      }
      ...
      try {
        if (mode === RESEARCH_MODE.AUTO_TRADE_RESEARCH) {
          ...
          if (!generatedResearch) {
            logger.info(
              { uid, mode },
              "⏭️ [SCHEDULER] Skipping AutoTradeEngine call - no research generated (safety guard)",
            );
            console.log(
              "🔥 [HARD_LOG] [AUTO_TRADE_GUARD] AutoTradeEngine call blocked - null research for user:",
              uid,
            );
            // Do NOT call AutoTradeEngine - set executionError for unified skip flow
            executionError = new Error(
              "AutoTradeEngine call skipped - no research generated",
            );
            ...
            console.log(
              "🔥 [HARD_LOG] [AUTO_TRADE_EXEC] Passing research to AutoTradeEngine for user:",
              uid,
              "research:",
              {
                symbol: generatedResearch.symbol,
            ...
            deepResearchResult =
              await autoTradeEngine.runAutoTradeResearchCycleSafe(
                uid,
                false,
                schedulerCycleId,
                generatedResearch,
          ...
          console.log(
            "🔥 [HARD_LOG] [TELEGRAM_EXEC] Using research for Telegram alerts for user:",
            uid,
          );
          ...
          logger.error(
            { uid, mode },
            "❌ [SCHEDULER] Unknown mode in processUserResearch",
          );
        ...
        logger.warn(
          { uid, mode, error: executeErr.message },
          "⚠️ [SCHEDULER] Execution failed, continuing with state update",
        );
      }
      ...
      console.log(
        "🔥 [HARD_LOG] [RESEARCH_RESULT_CHECK] Checking research result for user:",
        uid,
        "result exists:",
        !!deepResearchResult,
      );
      ...
      if (!deepResearchResult) {
        logger.info(
          { uid, mode },
          "📊 [RESEARCH] Research cycle completed - no signal generated",
        );
        console.log(
          "🔥 [HARD_LOG] [RESEARCH_NO_RESULT] Research returned null for user:",
          uid,
        );
        ...
        if (mode === RESEARCH_MODE.AUTO_TRADE_RESEARCH) {
          logger.info(
            { uid, mode },
            "[RESEARCH] No signal generated, no ERROR_CYCLE, AUTO_TRADE engine is the final authority.",
          );
        ...
        if (mode === RESEARCH_MODE.TELEGRAM_BACKGROUND_RESEARCH) {
          try {
            ...
            logger.info(
              { uid, mode, source: "TELEGRAM_BACKGROUND", symbol: null },
              "✅ [HISTORY] No-signal TELEGRAM_BACKGROUND history stored",
            );
          } catch (histError: any) {
            logger.warn(
              { uid, error: histError.message },
              "Failed to store TELEGRAM_BACKGROUND history for no-signal cycle",
            );
          }
        ...
        if (jobState) {
          ...
          console.log(
            "🔥 [HARD_LOG] [PROCESS_COMPLETE_NO_RESULT] processUserResearch() completed (no result) for user:",
            uid,
            "jobState updated",
          );
          // CRITICAL: Verify interval still exists - if missing, log warning (will be recreated on next check)
          if (!this.userIntervals.has(uid)) {
            ...
            console.log(
              "🔥 [HARD_LOG] [INTERVAL_MISSING] Interval missing after research cycle for user:",
              uid,
            );
          }
        }
        ...
        console.log(
          "🔥 [HARD_LOG] [RESEARCH_HAS_RESULT] Research returned valid result for user:",
          uid,
          "symbol:",
          deepResearchResult.symbol,
        );
        ...
        const finalAccuracy =
          typeof deepResearchResult.accuracy === "number"
            ? deepResearchResult.accuracy
            : parseFloat(deepResearchResult.accuracy as any) || 0;

        const finalAccuracyPercent = Math.round(
          finalAccuracy > 1 ? finalAccuracy : finalAccuracy * 100,
        );
        ...
        try {
          ...
          if (!isTop25) {
            ...
            if (jobState) {
              ...
              if (!this.userIntervals.has(uid)) {
                logger.warn(
                  ...
                  "⚠️ [SCHEDULER_IMMORTAL] Interval missing - will be recreated on next check",
                );
              }
            }
            ...
            try {
              ...
              logger.warn(
                { uid, error: updateError.message },
                "Failed to update state for top-25 filter",
              );
            }
          ...
          logger.error(
            {
              uid,
              coin,
              error: top25CheckError.message,
              stack: top25CheckError.stack,
            },
            "❌ [TOP_25_ERROR] Error checking top 25 - blocking alert for safety",
          );
          ...
          if (
            ...
            !deepResearchResult.result?.tradePlan
          ) {
            throw new Error(
              `INVARIANT VIOLATION: Signal is ${signal} but tradePlan missing in background research`,
            );
          }
          ...
          if (!context.symbol || context.accuracy == null || !context.signal) {
            throw new Error(
              `INVALID CONTEXT IN BACKGROUND: symbol=${context.symbol}, accuracy=${context.accuracy}, signal=${context.signal}`,
            );
          }
          ...
          if (
            ...
            throw new Error(
              `INVALID CONTEXT IN BACKGROUND: Signal is ${context.signal} but tradePlan missing`,
            );
          }
          ...
          console.log("🔥 [PIPELINE_GUARANTEE] Background context defined:", {
            ...
            contextTradePlanKeys: context.tradePlan
              ? Object.keys(context.tradePlan)
              : [],
          });
          ...
          console.log("🔥 [INVARIANT] Background research result:", {
            ...
            tradePlanKeys: context.tradePlan
              ? Object.keys(context.tradePlan)
              : [],
          });
          ...
          if (signal === "HOLD" && tradePlan !== null) {
            logger.error(
              { uid, coin, signal, tradePlan },
              "[TELEGRAM_VERIFY_ERROR] Signal is HOLD but tradePlan exists - this should never happen!",
            );
          } else if (
            signal !== "HOLD" &&
            finalAccuracyPercent >= 60 &&
            !tradePlan
          ) {
            logger.error(
              { uid, coin, signal, accuracy: finalAccuracyPercent },
          ...
          try {
            ...
            if (userSettings?.notifications?.whaleAlerts) {
              ...
              await checkWhaleAlerts(
                ...
                adapter,
                (uid, eventType, data) =>
                  autoTradeEngine.logTradeEvent(uid, eventType, data),
              );
            }
          } catch (whaleErr) {
          ...
          const minTrigger =
            accuracyTrigger?.min ??
            (typeof accuracyTrigger === "number" ? accuracyTrigger : 80);
          const maxTrigger = accuracyTrigger?.max ?? 100;
          const isInRange =
            finalAccuracyPercent >= minTrigger &&
            finalAccuracyPercent <= maxTrigger;
          ...
          logger.info(
            {
              ...
              reason: isInRange
                ? "Accuracy >= trigger"
                : `Accuracy ${finalAccuracyPercent}% outside range [${minTrigger}-${maxTrigger}]%`,
            },
          ...
          if (
            ...
            if (!telegramEnabled) {
              const skipReason =
                "TELEGRAM_DISABLED: Telegram Background Research not enabled";
              ...
              const skipReason =
                "MISSING_BOT_TOKEN: Telegram bot token not configured";
              ...
              const skipReason =
                "MISSING_CHAT_ID: Telegram chat ID not configured";
            ...
            if (telegramEnabled && hasBotToken && hasChatId) {
              ...
              try {
                ...
                logger.error(
                  { uid, symbol: coin, error: histErr.message },
                  "❌ [HISTORY] Failed to store Telegram background history",
                );
              ...
              logger.info(
                {
                  ...
                  status: "ATTEMPT",
                  reason:
                    "Accuracy >= trigger, alert sent on research completion",
                },
...
⚡ *Action:* Wait for higher confidence signal before trading.`;
                ...
                logger.info(
                  { uid, coin, accuracy: finalAccuracyPercent },
...
⚡ *Action:* Wait for higher confidence signal (>= 70%) before trading.`;
              ...
              if (telegramResult.success) {
                ...
                if (settings.telegramBotToken !== undefined)
                  cleanAlertSettings.telegramBotToken =
                    settings.telegramBotToken;
                if (settings.telegramChatId !== undefined)
                  cleanAlertSettings.telegramChatId = settings.telegramChatId;
                if (settings.researchFrequencyMinutes !== undefined)
                  cleanAlertSettings.researchFrequencyMinutes =
                    settings.researchFrequencyMinutes;
                if (settings.accuracyTrigger !== undefined)
                  cleanAlertSettings.accuracyTrigger = settings.accuracyTrigger;
                if (settings.selectedCoins !== undefined)
                  cleanAlertSettings.selectedCoins = settings.selectedCoins;

                await firestoreAdapter.saveBackgroundResearchSettings(
                  uid,
                  cleanAlertSettings,
                );
                ...
                logger.info(
                  {
                    ...
                    status: "SENT",
                    reason:
                      "Alert sent successfully - accuracy >= trigger on research completion",
                  },
              ...
              if (!telegramEnabled) reason = "Telegram disabled in settings";
                ...
                reason = `Accuracy ${finalAccuracyPercent}% outside trigger range`;
          ...
          if (
            ...
            try {
              ...
              logger.warn(
                { uid, error: histError.message },
                "Failed to store Telegram background research history",
              );
            }
          }
        }
        ...
        if (settings.telegramBotToken !== undefined)
          cleanUpdateSettings.telegramBotToken = settings.telegramBotToken;
        if (settings.telegramChatId !== undefined)
          cleanUpdateSettings.telegramChatId = settings.telegramChatId;
        if (settings.researchFrequencyMinutes !== undefined)
          cleanUpdateSettings.researchFrequencyMinutes =
            settings.researchFrequencyMinutes;
        if (settings.accuracyTrigger !== undefined)
          cleanUpdateSettings.accuracyTrigger = settings.accuracyTrigger;
        if (settings.selectedCoins !== undefined)
          cleanUpdateSettings.selectedCoins = settings.selectedCoins;
        ...
        console.log(
          "🔥 [HARD_LOG] [PROCESS_COMPLETE_FINAL] processUserResearch() completed successfully for user:",
          uid,
          "maxAccuracy:",
          maxAccuracy,
          "alertsSent:",
        ...
        if (didCandidateScanRun) {
          ...
          if (researchData && researchData.bestSymbol && researchData.bestSymbol !== "NO_RESEARCH") {
            ...
            accuracyWritten = researchData.bestAccuracy || 0;
            ...
            accuracyWritten = researchData.estimatedAccuracy || 0;
            skipReason = "Below accuracy threshold";
          } else {
            // Fallback - should not happen with didCandidateScanRun = true
            symbolWritten = "UNKNOWN_COIN";
            accuracyWritten = 0;
            skipReason = "Accuracy scan completed but no results";
          }
        } else {
      ...
      console.log(
        "🔥 [HARD_LOG] [PROCESS_ERROR_FINAL] processUserResearch() error for user:",
        uid,
        "error:",
        error?.message,
      );
      ...
      const errorSettings =
        await firestoreAdapter.getBackgroundResearchSettings(uid);
      const errorNow = admin.firestore.Timestamp.now();
      const errorFrequencyMinutes =
        errorSettings?.researchFrequencyMinutes || 5;
      const errorNextRunAt = admin.firestore.Timestamp.fromMillis(
        Date.now() + errorFrequencyMinutes * 60 * 1000,
      );
      ...
      if (!mode) {
        try {
          ...
          const autoTradeConfig = autoTradeConfigDoc.exists
            ...
            : null;
          const autoTradeEnabled = autoTradeConfig?.autoTradeEnabled === true;
          mode = autoTradeEnabled
            ? RESEARCH_MODE.AUTO_TRADE_RESEARCH
            : RESEARCH_MODE.TELEGRAM_BACKGROUND_RESEARCH;
        } catch (modeError: any) {
          logger.warn(
            { uid, error: modeError.message },
            "Failed to detect mode for error history",
          );
      ...
      if (mode !== RESEARCH_MODE.AUTO_TRADE_RESEARCH) {
        try {
          ...
          logger.warn(
            { uid, error: histError.message },
            "Failed to store history for error cycle",
          );
        }
      }
  ...
  private shouldSendTelegramAlert(
    lastAlert:
      | { timestamp: admin.firestore.Timestamp; accuracy: number }
      | undefined,
   ...
   */
  private async processTelegramBackgroundResult(
    uid: string,
    ...
    try {
      ...
      if (!isTop25) {
        ...
        try {
          ...
          logger.warn(
            ...
            "Failed to store top-25 block history",
          );
        }
        return;
      }
      ...
      logger.error(
        { uid, coin, error: top25CheckError.message },
        "❌ [TOP_25_ERROR] Error checking top 25",
      );
      return;
    }
    ...
    try {
      ...
      if (userSettings?.notifications?.whaleAlerts) {
        // Get adapter through autoTradeEngine initialization
        const adapter = await autoTradeEngine.initializeAdapter(uid);
        await checkWhaleAlerts(uid, coin, adapter, (uid, eventType, data) =>
          autoTradeEngine.logTradeEvent(uid, eventType, data),
        );
      }
    } catch (whaleErr) {
    ...
    if (isInRange) {
      await this.sendTelegramBackgroundAlert(
        uid,
        coin,
        signal,
    ...
    try {
      ...
      logger.warn(
        ...
        "Failed to store Telegram background history",
      );
    }
   ...
   */
  private async sendTelegramBackgroundAlert(
    uid: string,
    coin: string,
    signal: string,
    accuracy: number,
    ...
    try {
      ...
      if (result.success) {
        logger.info(
          { uid, coin, accuracy },
          "✅ [TELEGRAM] Background alert sent",
        );
      } else {
        logger.warn(
          { uid, coin, error: result.error },
  ...
  private async hasRequiredPrimaryAPIs(uid: string): Promise<boolean> {
    try {
      ...
      const hasCryptoCompare =
        cryptocompare?.enabled === true && !!cryptocompare?.apiKey;
   ...
   */
  private async hasUsableExchangeAPIs(
    uid: string,
