...
export class AutoTradeEngine {
  ...
  async saveConfig(uid: string, config: Partial<AutoTradeConfig>): Promise<AutoTradeConfig> {
    try {
      ...
      firestoreAdapter.guardAgainstIllegalWrites(`users/${uid}/autoTradeConfig/current`, finalSanitized, 'background_job');
  ...
  async executeTrade(uid: string, signal: TradeSignal, skipConfirmationCheck: boolean = false): Promise<TradeExecution> {
    ...
    const accuracyValidation = validateAccuracyForExecution(signal.accuracy, settings, skipConfirmationCheck);
    ...
    const modelParams = await calculateDynamicParams(uid, signal.accuracy, (signal as any).volatilityClassification || 'low', 50);
  ...
  private async runAutoTradeExecutionCycle(
    ...
    if (!exchangeUsability.usable) {
      if (exchangeUsability.reason === 'not_connected') {
        // GLOBAL INVARIANT: If reason is 'not_connected', IMMEDIATELY return after history save
        logger.info({ uid }, '💡 [SOFT_STATE] Auto-Trade execution cycle SKIPPED - exchange not connected (Missing exchangeConfig)');
      ...
      if (exchangeUsability.reason === 'not_connected') {
        throw new Error(`FATAL_INVARIANT_VIOLATION: 'not_connected' reached BLOCKED logic in engine (execution) | uid=${uid}`);
      }
  ...
  async runAutoTradeResearchCycle(uid: string, skipHistoryStorage: boolean = false, cycleId?: string): Promise<ResearchDataResult | null> {
    ...
    if (!exchangeUsability.usable) {
      if (exchangeUsability.reason === 'not_connected') {
        // GLOBAL INVARIANT: If reason is 'not_connected', IMMEDIATELY return after history save
        logger.info({ uid }, '💡 [SOFT_STATE] Auto-Trade research cycle SKIPPED - exchange not connected (Missing exchangeConfig)');
      ...
      // RUNTIME ASSERTION: If reason is 'not_connected' here, it's a critical logic failure
      if (exchangeUsability.reason === 'not_connected') {
        throw new Error(`FATAL_INVARIANT_VIOLATION: 'not_connected' reached BLOCKED logic in engine (research) | uid=${uid}`);
      }
    ...
    try {
      ...
      const accuracyValidation = validateAccuracyForExecution(accuracy, settings, false); // Always use auto-trade threshold
      ...
      if (!skipHistoryStorage) {
        try {
          ...
          logger.info({ uid }, '✅ [HISTORY] ERROR history saved for failed trade cycle');
        } catch (histError: any) {
          logger.error({ uid, error: histError.message }, '❌ [HISTORY] Failed to save ERROR history for failed cycle');
        }
      }
