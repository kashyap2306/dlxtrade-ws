...
export async function fetchCoinMarketCapMarketData(symbol: string, apiKey?: string): Promise<any> {
  ...
  try {
    ...
    if (!coinData) {
      console.log('[CoinMarketCap] NO DATA in Listings - Trying Quotes endpoint as fallback:', cmcSymbol);

      // Fallback: Try specific quote endpoint for the symbol
      ...
      if (quotesResponse && quotesResponse.data && quotesResponse.data.data && quotesResponse.data.data[cmcSymbol]) {
        const specificCoin = quotesResponse.data.data[cmcSymbol];
        // Construct result from specific quote
        return {
          provider: 'coinmarketcap',
          success: true,
          symbol,
          hasData: true,
          price: specificCoin.quote?.USD?.price || 0,
          volume24h: specificCoin.quote?.USD?.volume_24h || 0,
          marketCap: specificCoin.quote?.USD?.market_cap || 0,
          change24h: specificCoin.quote?.USD?.percent_change_24h || 0,
          change7d: specificCoin.quote?.USD?.percent_change_7d || 0,
          change30d: specificCoin.quote?.USD?.percent_change_30d || 0,
          latency: Date.now() - startTime
        };
      }
      ...
      return {
        provider: 'coinmarketcap',
        success: false,
        error: `Symbol ${cmcSymbol} not found in listings or quotes`,
        marketData: {},
        liquidity: 0,
        marketCap: 0,
        latency: Date.now() - startTime,
