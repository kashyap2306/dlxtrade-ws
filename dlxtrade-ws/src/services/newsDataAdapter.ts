...
export async function fetchNewsData(apiKey: string, symbol?: string): Promise<any> {
  ...
  try {
    ...
    let normalizedArticles = articles.map((article: any) => ({
      title: article.title || 'Untitled',
      url: article.link || article.url || '#',
      publishedAt: article.pubDate || article.published_at || new Date().toISOString(),
      summary: article.description || article.title || article.summary || '',
      source: article.source_id || article.source?.name || article.source || 'Unknown'
    }));

    // If no articles found, add a placeholder
    if (normalizedArticles.length === 0) {
      normalizedArticles.push({
        title: 'Market Analysis: Monitoring latest developments...',
        url: '#',
        publishedAt: new Date().toISOString(),
        summary: 'Our systems are currently monitoring the latest market narratives and social sentiment for this asset. Real-time updates will appear as soon as they are verified.',
    ...
    // For other errors (including final 429), return placeholder news to not break research
    console.log('[NewsData] FALLBACK - Error occurred or no results, returning placeholder news');
    const placeholder = {
      title: 'Market Analysis: Monitoring latest developments...',
      url: '#',
      publishedAt: new Date().toISOString(),
    ...
    return {
      ...
      sentiment: 0.5,
      latency: Date.now() - startTime,
      message: "Using fallback placeholder news"
    };
  }
}
