...
import { seedProviderIntegrations } from '../services/userOnboarding';
...
async function seedDemoUsers(db: admin.firestore.Firestore): Promise<void> {
  try {
    ...
    for (let i = 1; i <= DEMO_USERS_COUNT; i++) {
      ...
      // Initialize subcollection settings/current
      await db.collection('users').doc(uid).collection('settings').doc('current').set({
        symbol: 'BTCUSDT',
        quoteSize: 10,
        adversePct: 1.5,
        cancelMs: 60000,
      ...
      // CRITICAL: Ensure demo users get provider integrations seeded
      // This ensures demo_user_* accounts have complete provider structure for AutoTrade diagnostics
