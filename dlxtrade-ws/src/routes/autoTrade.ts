...
export async function autoTradeRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/diagnostic-check', {
    ...
    try {
      ...
      if (!exchangeResolved && !exchangeDecryptionFailed) {
        try {
      ...
      if (exchangeDecryptionFailed) {
        ...
        diagnostics.systemChecks.exchangeConnected = {
          status: 'FAIL',
          message: `❌ Exchange API key decryption failed - ${errorMessage}. Please re-enter your exchange API keys.`,
          value: false,
          exchangeName: null,
          decryptionFailed: true,
          userAction: 'Re-enter exchange API keys in Settings',
          technicalDetails: 'ENCRYPTION_SECRET mismatch detected'
        };
      } else {
  ...
  fastify.get('/status', {
    ...
    try {
      ...
      if (exchangeData) {
        const hasApiKey = !!exchangeData.apiKeyEncrypted && exchangeData.apiKeyEncrypted.length > 0;
