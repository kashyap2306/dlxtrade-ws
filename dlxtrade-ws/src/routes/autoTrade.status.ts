...
const autoTradeConfigCache = new Map<
  string,
  {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        if (!user?.uid) {
          console.log(
            "[ROUTE_EXIT] /auto-trade/status (no uid)",
            Date.now() - startTime,
            "ms",
          );
        ...
        const userJobScheduled =
          schedulerInstance?.isUserScheduled(user.uid) || false;
        ...
        console.log("[ROUTE_EXIT] /auto-trade/status", duration, "ms", {
          exchangeConnected,
          autoTradeEnabled,
        });
        ...
        if (!reply.sent) {
          return reply.send({
            ...
            providerStatus: hasUsableProviders
              ? "CONFIGURED"
              : "MISSING_PROVIDERS",
            providerMessage: hasUsableProviders ? null : providerStatusMessage,
            diagnostics: {
              marketDataReady: hasUsableProviders,
              newsReady: hasUsableProviders,
            },
        ...
        exchangeReason =
          "Status check failed: " +
          (err.message || "Unable to determine auto-trade status");
      }
    },
  );
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        let lastTradeReason = autoTradeEnabled
          ? "Auto-trade enabled and monitoring"
          : "Auto-trade disabled";
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        if (!user?.uid) {
          console.log(
            "[ROUTE_EXIT] /auto-trade/config (no uid)",
            Date.now() - startTime,
            "ms",
          );
          return reply.code(401).send({ error: "Authentication required" });
        ...
        const schedulerIndicatesEnabled =
          jobState?.mode === "AUTO_TRADE_RESEARCH";
        ...
        const duration = Date.now() - startTime;
        console.log(
          "[ROUTE_EXIT] /auto-trade/config",
          duration,
          "ms - instant response with cached config",
        );
        ...
        if (!activeCacheRefreshes.has(user.uid)) {
          ...
          setImmediate(async () => {
            try {
              ...
              const freshSettings = bgSettingsDoc?.exists
                ? bgSettingsDoc.data()
                : {};
              ...
              if (freshConfig || freshSettings) {
                ...
                console.log(
                  "[CONFIG_CACHE_UPDATE] Updated cache with fresh Firestore data for user:",
                  user.uid,
                );
              }
            } catch (err: any) {
              ...
              console.warn(
                "[CONFIG_CACHE_UPDATE_FAILED] Could not update cache:",
                err.message,
              );
            } finally {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const finalMode =
          body.mode || existingConfig?.mode || "AUTO_TRADE_RESEARCH";
        ...
        logger.error(
          ...
          "Error updating auto-trade config",
        );
