...
export async function engineRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/start', {
    ...
    try {
      ...
      const integrations = await firestoreAdapter.getEnabledIntegrations(user.uid, "user_request");
  ...
  fastify.post('/engine/toggle', {
    ...
    try {
      if (enabled) {
        ...
        const configDoc = await db.collection('users').doc(user.uid).collection('autoTradeConfig').doc('current').get();
        const configData = configDoc.exists ? configDoc.data() : null;
        const autoTradeEnabled = configData?.autoTradeEnabled ?? false;
        ...
        if (autoTradeEnabled) {
          ...
          await userEngineManager.createUserEngine(
            user.uid,
            decrypt(exchangeCredentials.apiKeyEncrypted, "user_request"),
            decrypt(exchangeCredentials.secretKeyEncrypted || exchangeCredentials.secretEncrypted, "user_request"),
