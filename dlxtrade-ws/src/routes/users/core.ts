...
export async function coreUserRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/:uid/features', {
    ...
    try {
      ...
      console.error("[GET_FEATURES_ERROR] isAdmin check failed:", err);
      isAdmin = false;
    }
    ...
    console.log("[GET_FEATURES_EXIT] users/:uid/features");
    return reply.send({
      success: true,
      features: {
        news: true,
  ...
  // POST /api/users/:uid/logout-all - Logout all sessions (revoke all refresh tokens)
  fastify.post('/:uid/logout-all', {
    preHandler: [fastify.authenticate],
  }, async (request: FastifyRequest<{ Params: { uid: string } }>, reply: FastifyReply) => {
    try {
      const { uid: paramUid } = request.params;
      ...
      await admin.auth(adminApp).revokeRefreshTokens(targetUid);

      // Log the action
      await firestoreAdapter.logActivity(targetUid, 'ALL_SESSIONS_LOGGED_OUT', {
      ...
      return reply.code(500).send({ error: 'Failed to logout all sessions' });
    }
  });
  ...
  fastify.get('/:uid/performance-stats', {
    ...
    try {
      ...
      const authUid = getAuthUid(request);
      if (!authUid) {
        routeExitLog('GET /performance-stats (no auth)', startTime);
        return reply.code(401).send({ error: 'Authentication required' });
      }

      // Users can only view their own performance stats unless they're admin
      const isAdmin = await firestoreReadWithTimeout(
        () => firestoreAdapter.isAdmin(authUid),
        false,
        'isAdmin-check'
      );
      if (paramUid !== authUid && !isAdmin) {
        routeExitLog('GET /performance-stats (denied)', startTime);
        return reply.code(403).send({ error: 'Access denied' });
      }
      ...
      // If disabled, return lightweight response immediately (no heavy queries)
      // Note: getFirebaseAdmin is already imported at the top of the file
      const configDoc = await getFirebaseAdmin().firestore()
        .collection('users').doc(targetUid).collection('autoTradeConfig').doc('current').get();
      const config = configDoc?.exists ? configDoc.data() : {};
      const autoTradeEnabled = config?.autoTradeEnabled ?? false;
      
      if (!autoTradeEnabled) {
        logger.debug({ uid: targetUid }, '⏭️ [EARLY_EXIT] /performance-stats - auto-trade disabled, returning lightweight response');
        routeExitLog('GET /performance-stats (early-exit)', startTime);
        return {
          dailyPnL: 0,
          allTimePnL: 0,
          winRate: 0,
      ...
      // Calculate performance stats (in-memory, fast)
      ...
      routeExitLog('GET /performance-stats', startTime);
      return {
      ...
      routeExitLog('GET /performance-stats (error)', startTime);
      return { dailyPnL: 0, allTimePnL: 0, winRate: 0, totalTrades: 0 };
    }
  });
