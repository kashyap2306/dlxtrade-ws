      ...
      try {
        ...
        if (enabled) {
          try {
            ...
            // This is an action route that might enable/disable auto-trade
            // We want to allow decryption and key validation for explicit user actions
        ...
        try {
          if (enabled) {
            ...
            if (isBackgroundPaused()) {
              console.log(
                "🔥 [HARD_LOG] [AUTO_TRADE_ENGINE_RESUMED_AFTER_TOGGLE] Background tasks paused, ensuring user registration proceeds",
              );
              logger.info(
                { uid: user.uid },
                "🔥 [AUTO_TRADE_ENGINE_RESUMED_AFTER_TOGGLE] Background tasks paused but proceeding with user registration",
              );
            }
            ...
            logger.info(
              { uid: user.uid },
              "🔗 [AUTO_TRADE_TOGGLE] Registering user with scheduler",
            );
            const scheduleResult =
              await backgroundResearchScheduler.ensureUserResearchScheduled(
                user.uid,
              );
            ...
            logger.info(
              { uid: user.uid },
              "🔗 [AUTO_TRADE_TOGGLE] Unregistering user from scheduler",
            );
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        return reply
          .code(500)
          .send({ error: err.message || "Error resetting circuit breaker" });
      }
    },
  );
}
