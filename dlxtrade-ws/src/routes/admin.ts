...
export async function adminRoutes(fastify: FastifyInstance) {
  ...
  fastify.delete('/users/:uid', {
    ...
    try {
      ...
      return reply.code(500).send({ success: false, error: err.message || "Failed to delete user" });
    }
  });
  ...
  // 🔴 TEMPORARY ADMIN ROUTE - DELETE ALL USER INTEGRATIONS
  // DELETE /internal/delete-all-integrations
  // This endpoint deletes all integrations collections for all users
  // WARNING: This operation is IRREVERSIBLE!
  fastify.delete('/internal/delete-all-integrations', {
    preHandler: [fastify.adminAuth],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      console.log('🔴 ===================================================');
      console.log('🔴 DANGER: MANUAL DELETION REQUEST RECEIVED');
      console.log('🔴 ===================================================');
      console.log('🔴 WARNING: This will delete ALL user integrations!');
      console.log('🔴 This operation is IRREVERSIBLE!');
      console.log('🔴 Users will need to re-enter their API keys afterwards.');
      console.log('🔴 ===================================================');

      // Execute the deletion
      await deleteAllUserIntegrations();

      console.log('🔴 ===================================================');
      console.log('🔴 DELETION COMPLETED VIA API ENDPOINT');
      console.log('🔴 ===================================================');

      return reply.send({
        success: true,
        message: 'All user integrations have been deleted successfully',
        warning: 'This operation is irreversible. Users must re-enter their API keys.',
        timestamp: new Date().toISOString()
      });

    } catch (error: any) {
      console.error('❌ API endpoint deletion failed:', error.message);
 ...
 * Deletes all integrations collections for all users
 * This is required when API keys were encrypted with old ENCRYPTION_SECRET
 * and cannot be decrypted anymore.
 *
 * WARNING: This operation is IRREVERSIBLE!
 * All user API integrations will be permanently deleted.
 * Users will need to re-enter their API keys after this operation.
 */
async function deleteAllUserIntegrations() {
  console.log('🔴 ===================================================');
  console.log('🔴 DANGER: DELETE ALL USER INTEGRATIONS');
  console.log('🔴 ===================================================');
  console.log('🔴 WARNING: This operation is IRREVERSIBLE!');
  console.log('🔴 All user API integrations will be permanently deleted.');
  console.log('🔴 Users will need to re-enter their API keys afterwards.');
  console.log('🔴 ===================================================');

  // Wait for confirmation (in production, this would be an interactive prompt)
  ...
  try {
    ...
    for (const userDoc of usersSnapshot.docs) {
      ...
      try {
        const integrationsRef = usersRef.doc(userId).collection('integrations');
        const integrationsSnapshot = await integrationsRef.get();

        if (!integrationsSnapshot.empty) {
          console.log(`🗑️  Deleting ${integrationsSnapshot.size} integrations for user: ${userId}`);

          // Delete all integration documents in batch
          ...
          deletedCount++;
          console.log(`✅ Successfully deleted integrations for user: ${userId}`);
        } else {
          console.log(`⏭️  No integrations found for user: ${userId}`);
        }
      } catch (error: any) {
        console.error(`❌ Failed to delete integrations for user ${userId}:`, error.message);
      }
    }
    ...
    console.log(`🗑️  Deleted integrations for: ${deletedCount} users`);
    console.log('✅ Operation completed successfully');
    console.log('🔄 Users will need to re-enter their API keys through the UI');
    console.log('🔴 ===================================================');

  } catch (error: any) {
    console.error('💥 CRITICAL ERROR during deletion process:', error.message);
    console.error('🔴 Operation may have failed partially or completely');
    throw error;
  }
}
