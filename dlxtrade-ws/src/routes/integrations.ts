...
export async function getUserIntegrations(uid: string) {
  ...
  if (!uid) {
    throw new Error("getUserIntegrations called with null/undefined uid");
  }
    ...
    try {
      ...
      if (type) {
        ...
        for (const [key, integration] of Object.entries(integrations)) {
          ...
          } else if (type === 'news' && ['newsdata', 'cryptopanic', 'reddit', 'cointelegraph', 'altcoinbuzz', 'gnews', 'marketaux', 'webz', 'coinstatsnews', 'newscatcher', 'cryptocomparenews'].includes(key)) {
            filtered[key] = {
              enabled: integration.enabled,
              apiKey: integration.apiKey ? maskKey(integration.apiKey) : null,
            };
