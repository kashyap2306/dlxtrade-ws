...
export async function notificationsRoutes(fastify: FastifyInstance) {
  ...
  // GET /api/notifications - Get user notifications (HARDENED: 200ms timeout)
  fastify.get('/', {
    ...
    if (!user?.uid) {
      logger.warn({}, 'GET /notifications - missing uid, returning safe default');
      return reply.send([]);
    }
    ...
    try {
      ...
      const firestoreOperation = async () => {
        return await firestoreAdapter.getUserNotificationsFromSubcollection(user.uid, limit);
      };

      const notifications = await Promise.race([firestoreOperation(), timeoutPromise]) as any[];
      logger.info({ uid: user.uid, count: notifications?.length || 0, duration: Date.now() - startTime }, 'GET /notifications success');
      return reply.send(notifications || []);
    } catch (err: any) {
      logger.warn({ uid: user.uid, error: err.message, duration: Date.now() - startTime }, 'GET /notifications timeout/error - returning safe default');
