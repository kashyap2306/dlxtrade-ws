...
export async function diagnosticsRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/test', {
    ...
    const authUid = (request as any).userId || user?.uid;

    // AUTH UID ISOLATION: Log and enforce single UID usage
    console.log(`[DIAGNOSTICS_AUTH_UID_ISOLATED] Using isolated UID: ${authUid}`);
    if (!authUid) {
      console.error("[DIAGNOSTICS_AUTH_UID_MISSING] No authenticated user for diagnostics");
