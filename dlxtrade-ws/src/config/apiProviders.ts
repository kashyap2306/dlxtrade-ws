...
// Estimated free tier limits for primary providers
export const ESTIMATED_FREE_TIER_LIMITS: Record<string, { daily: number; monthly: number }> = {
...
export const ALL_NEWS_PROVIDER_IDS = new Set([
  API_PROVIDERS_CONFIG.news.primary.id,
  ...API_PROVIDERS_CONFIG.news.backups.map(p => p.id)
]);

export const SYSTEM_NEWS_PROVIDER_IDS = new Set(
  [API_PROVIDERS_CONFIG.news.primary, ...API_PROVIDERS_CONFIG.news.backups]
