...
class UserIntegrationsMigrationVerifier {
  ...
  private async verifyUserDeepResearch(user: any, semaphore: Semaphore): Promise<void> {
    ...
    try {
      ...
      // Select optimal symbol batch (up to 5 symbols)
      const { batchSymbols } = await deepResearchEngine.selectOptimalSymbolBatch(user.uid, 5);
      ...
      try {
        // Run research on first symbol only (for verification)
        const symbol = batchSymbols[0];
        const result = await deepResearchEngine.runNormalizedDeepResearch(symbol, user.uid);

        // Extract sample prices
        const samplePrices: Record<string, number> = {};
        if (result.marketOverview.price > 0) {
          samplePrices[symbol] = result.marketOverview.price;
        }
        ...
        const verificationResult: DeepResearchVerificationResult = {
          ...
          successRate: result.deepAnalysis.confidencePercent / 100,
          samplePrices,
          logSnippets,
